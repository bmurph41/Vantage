...
import DocketTabs from "../components/DocketTabs";
...
export default function Dashboard() {
  ...
  const getActiveTab = () => {
    if (location === "/docket/market-intelligence") return "market-intelligence";
    if (location === "/docket/m&a-spotlight") return "m&a-spotlight";
    if (location === "/docket/saved") return "saved";
    if (location === "/docket/watchlist") return "watchlist";
    if (location === "/docket/email-alerts") return "email-alerts";
  ...
  const handleNotificationClick = () => {
    setLocation('/docket/email-alerts');
  };

  const handleSettingsClick = () => {
    setLocation('/docket/sources');
  };

  const handleArticleManagementClick = () => {
    setLocation('/docket/article-management');
  };

  // Check if we're on a settings/notification page (show Articles button)
  const isSettingsPage = location === "/docket/notifications" || location === "/docket/sources";

  // Show notifications page for email setup
  if (location === "/docket/notifications") {
    return (
      <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-950">
        <DocketHeader
          newArticlesCount={systemStats?.newArticlesToday || 0}
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-950">
      ...
      <div className="flex-1 overflow-auto">
        <DocketTabs activeTab={getActiveTab()} />
      </div>
    </div>
  );
}
