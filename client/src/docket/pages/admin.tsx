...
export default function AdminPage() {
  ...
  const { data: rssSources = [], isLoading: rssSourcesLoading } = useQuery({
    queryKey: ["/api/docket/rss-sources"],
  ...
  const { data: articles = [], isLoading: articlesLoading } = useQuery({
    queryKey: ["/api/docket/articles", { limit: 50, offset: 0, sortBy: "newest", category: categoryFilter, source: sourceFilter }],
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/docket/rss-sources"] });
                ...
                <DialogContent>
                  ...
                  <DialogFooter>
                    <Button
                      onClick={async () => {
                        try {
                          const response = await apiRequest("/api/docket/admin/backfill", {
                            method: "POST",
                            body: JSON.stringify({
                              monthsBack: 6,
                    ...
                    <Button
                      onClick={async () => {
                        try {
                          ...
                          const response = await apiRequest("/api/docket/deals/backfill-from-articles", {
                            method: "POST",
                            body: JSON.stringify({
