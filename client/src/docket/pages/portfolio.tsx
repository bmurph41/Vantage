...
export default function PortfolioCompaniesPage() {
  ...
  const { data: companies = [], isLoading } = useQuery<PortfolioCompany[]>({
    queryKey: ['/api/docket/portfolio-companies'],
  });

  const { data: companyArticles = [] } = useQuery<Article[]>({
    queryKey: ['/api/docket/portfolio-companies', expandedCompany, 'articles'],
    queryFn: async () => {
      if (!expandedCompany) return [];
      const response = await fetch(`/api/docket/portfolio-companies/${expandedCompany}/articles?limit=20`, {
        credentials: 'include',
      });
  ...
  const { data: crmSearchResults = [] } = useQuery<CrmCompanyMatch[]>({
    queryKey: ['/api/docket/crm-companies/search', crmSearchQuery],
    queryFn: async () => {
      if (!crmSearchQuery || crmSearchQuery.length < 2) return [];
      const response = await fetch(`/api/docket/crm-companies/search?q=${encodeURIComponent(crmSearchQuery)}`, {
        credentials: 'include',
      });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await fetch('/api/docket/portfolio-companies', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    onSuccess: (newCompany) => {
      queryClient.invalidateQueries({ queryKey: ['/api/docket/portfolio-companies'] });
  ...
  const linkMutation = useMutation({
    mutationFn: async ({ portfolioId, crmCompanyId }: { portfolioId: string; crmCompanyId: string }) => {
      const response = await fetch(`/api/docket/portfolio-companies/${portfolioId}/link-crm`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const unlinkMutation = useMutation({
    mutationFn: async (portfolioId: string) => {
      const response = await fetch(`/api/docket/portfolio-companies/${portfolioId}/unlink-crm`, {
        method: 'DELETE',
        credentials: 'include',
      });
  ...
  const createCrmMutation = useMutation({
    mutationFn: async ({ portfolioId, crmData }: { portfolioId: string; crmData: any }) => {
      const response = await fetch(`/api/docket/portfolio-companies/${portfolioId}/create-crm`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const response = await fetch(`/api/docket/portfolio-companies/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
        credentials: 'include',
  ...
  const checkForMatches = async () => {
    ...
    try {
      const response = await fetch('/api/docket/portfolio-companies/match-suggestions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
