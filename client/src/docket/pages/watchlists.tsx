...
export default function WatchlistsPage() {
  ...
  const { data: watchlists = [], isLoading } = useQuery<Watchlist[]>({
    queryKey: ['/api/docket/watchlists'],
  });

  // Fetch articles for expanded watchlist
  const { data: watchlistArticles = [] } = useQuery<Article[]>({
    queryKey: ['/api/docket/watchlists', expandedWatchlist, 'articles'],
    queryFn: async () => {
      if (!expandedWatchlist) return [];
      const response = await fetch(`/api/docket/watchlists/${expandedWatchlist}/articles?limit=20`, {
        credentials: 'include',
      });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: {
      ...
      const response = await fetch('/api/docket/watchlists', {
        method: 'POST',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/docket/watchlists'] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<Watchlist> }) => {
      const response = await fetch(`/api/docket/watchlists/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
