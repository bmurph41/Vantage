...
export default function SavedSearchesPage() {
  ...
  const { data: currentUser, isLoading: userLoading } = useQuery<any>({
    queryKey: ["/api/docket/user/current"],
  });
  ...
  const updatePreferencesMutation = useMutation({
    mutationFn: async (data: EmailSettingsForm) => {
      const response = await fetch("/api/docket/user/notification-preferences", {
        method: "PATCH",
        headers: csrfHeaders(),
        credentials: "include",
        body: JSON.stringify({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/docket/user/current"] });
      toast({
  ...
  const testEmailMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/docket/test-email", {
        method: "POST",
        headers: csrfHeaders(),
        credentials: "include",
      });
  ...
  const { data: searches = [], isLoading } = useQuery<SavedSearch[]>({
    queryKey: ['/api/docket/saved-searches'],
  });

  const { data: searchResults = [] } = useQuery<Article[]>({
    queryKey: ['/api/docket/saved-searches', expandedSearch, 'results'],
    queryFn: async () => {
      if (!expandedSearch) return [];
      const response = await fetch(`/api/docket/saved-searches/${expandedSearch}/results?limit=20`, {
        credentials: 'include',
      });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await fetch('/api/docket/saved-searches', {
        method: 'POST',
        headers: csrfHeaders(),
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/docket/saved-searches'] });
      closeAlertDialog();
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<SavedSearch> }) => {
      const response = await fetch(`/api/docket/saved-searches/${id}`, {
        method: 'PATCH',
        headers: csrfHeaders(),
        body: JSON.stringify(data),
        credentials: 'include',
