...
export default function DealsPage() {
  ...
  const createDealMutation = useMutation({
    mutationFn: async (data: typeof dealForm) => {
      return apiRequest("POST", "/api/docket/deals", data);
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ predicate: (query) => 
        String(query.queryKey[0]).startsWith("/api/docket/deals")
      });
    },
    onError: (error: any) => {
  ...
  const syncFromSalesCompsMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", "/api/docket/deals/sync-from-sales-comps", {});
      return response;
    },
  ...
  useEffect(() => {
    ...
    const newUrl = `/docket/m&a-spotlight${newSearch ? `?${newSearch}` : ''}`;
  ...
  const apiUrl = `/api/docket/deals${queryString ? `?${queryString}` : ''}`;
  ...
  const analyticsUrl = `/api/docket/deals/analytics${analyticsQueryString ? `?${analyticsQueryString}` : ''}`;
  ...
  const maArticlesUrl = `/api/docket/ma-spotlight-articles${maArticlesParams.toString() ? `?${maArticlesParams.toString()}` : ''}`;
  ...
  const maAnalyticsUrl = `/api/docket/ma-spotlight-analytics${maAnalyticsParams.toString() ? `?${maAnalyticsParams.toString()}` : ''}`;
  ...
  const handleExportCSV = () => {
    ...
    const filename = `docket-deals-${timestamp}.csv`;
                        ...
                        <div className="mt-4 pt-4 border-t">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                              <Link href={`/docket?article=${article.id}`}>
