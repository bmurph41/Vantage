...
export default function ArticleCard({ article, featured = false }: ArticleCardProps) {
  ...
  const { data: bookmarkStatus } = useQuery({
    queryKey: ['/api/docket/bookmarks', article.id, 'status'],
    queryFn: () => checkBookmarkStatus(article.id),
    staleTime: 30000,
  });

  const { data: readingListStatus } = useQuery({
    queryKey: ['/api/docket/reading-list', article.id, 'status'],
  ...
  const bookmarkMutation = useMutation({
    ...
    onSuccess: (_, action) => {
      queryClient.invalidateQueries({ queryKey: ['/api/docket/bookmarks'] });
      queryClient.invalidateQueries({ queryKey: ['/api/docket/bookmarks', article.id, 'status'] });
      toast({
  ...
  const readingListMutation = useMutation({
    ...
    onSuccess: (_, { action }) => {
      queryClient.invalidateQueries({ queryKey: ['/api/docket/reading-list'] });
      queryClient.invalidateQueries({ queryKey: ['/api/docket/reading-list', article.id, 'status'] });
      toast({
