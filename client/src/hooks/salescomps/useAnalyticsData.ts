...
import { salesCompsApi, type CompsResponse } from "@/lib/salescomps/api";
...
import { deriveEffectiveCapRate } from "@/lib/salescomps/capRateUtils";
import { calculateStats, groupByStats, formatCurrency, formatPercent } from "@/lib/salescomps/stats";
...
export function useAnalyticsData(filters: FilterState): AnalyticsData {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const response = await salesCompsApi.getComps({
        ...queryParams,
        page: pageParam,
      });
      return response;
 ...
 */
function createQuarterlyTimeSeries(allComps: SalesComp[]) {
  const quarterlyData: { [key: string]: { comps: SalesComp[], prices: number[] } } = {};
 ...
 */
function createMonthlyTimeSeries(allComps: SalesComp[]) {
  const monthlyData: { [key: string]: { comps: SalesComp[], prices: number[] } } = {};
 ...
 */
function createRegionalAnalysis(allComps: SalesComp[]) {
  // Define US regions
  ...
  const regionData: { [region: string]: { comps: SalesComp[], prices: number[], states: Set<string> } } = {};
 ...
 */
function createPriceTranches(allComps: SalesComp[]) {
  const tranches = [
  ...
  const trancheData = tranches.map(tranche => ({ ...tranche, comps: [] as SalesComp[], prices: [] as number[] }));
 ...
 */
function createOwnershipAnalysis(allComps: SalesComp[]) {
  const ownerData: { [owner: string]: { comps: SalesComp[], prices: number[] } } = {};
