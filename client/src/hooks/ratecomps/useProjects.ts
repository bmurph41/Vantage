...
import { projectsApi, type ProjectWithStats, type ProjectCompsResponse } from "@/lib/ratecomps/api";
import { queryKeys } from "@/lib/ratecomps/queryKeys";
import { useToast } from "@/hooks/use-toast";
import { isUnauthorizedError } from "@/lib/ratecomps/authUtils";
import type { RcProject, InsertRcProject, UpdateRcProject, InsertRcProjectComp } from "@shared/schema";
...
export function useCreateProject(onProjectCreated?: (project: RcProject, recommendations: any) => void) {
  const queryClient = useQueryClient();
  const { toast } = useToast();

  return useMutation({
    mutationFn: (projectData: InsertRcProject) => projectsApi.createProject(projectData),
    onSuccess: (response) => {
...
export function useUpdateProject() {
  ...
  return useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: UpdateRcProject }) => 
...
export function useAddCompToProject() {
  ...
  return useMutation({
    mutationFn: ({ projectId, compData }: { projectId: string; compData: InsertRcProjectComp }) => 
      projectsApi.addCompToProject(projectId, compData),
    onSuccess: (_, { projectId }) => {
      toast({
        title: "Success",
        description: "Rate comp added to project successfully",
      });
    ...
    onError: (error) => {
      ...
      if (message.includes("already added")) {
        toast({
          title: "Already Added",
          description: "This rate comp is already in the project",
          variant: "destructive",
        });
        return;
      }
...
export function useBulkAddCompsToProject() {
  ...
  return useMutation({
    ...
    onSuccess: (result, { projectId }) => {
      ...
      let message = `Added ${added} rate comps to project`;
      if (skipped > 0) {
...
export function useBulkRemoveCompsFromProject() {
  ...
  return useMutation({
    ...
    onSuccess: (result, { projectId }) => {
      ...
      toast({
        title: "Success",
        description: `Removed ${removed} rate comps from project`,
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.projects.comps(projectId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.projects.all });
    },
...
export function useRemoveCompFromProject() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { projectId }) => {
      toast({
        title: "Success",
        description: "Rate comp removed from project successfully",
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.projects.comps(projectId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.projects.all });
    },
