...
import {
  ...
  useDealTeamStats, useTaskBreakdown, useLifetimeTaskStats,
  DdChecklistItem, DdChecklistSection as SectionType, DdChecklistItemPeriod, DealTeamContact,
  DealTeamMemberStats, TaskBreakdownUser, LifetimeUserStats,
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="checklist" className="w-full">
          <TabsList className="mb-4">
            <TabsTrigger value="checklist" className="text-sm">
              <ClipboardList className="h-3.5 w-3.5 mr-1.5" />Checklist
            </TabsTrigger>
            <TabsTrigger value="breakdown" className="text-sm">
              <BarChart3 className="h-3.5 w-3.5 mr-1.5" />Task Breakdown
            </TabsTrigger>
          </TabsList>
        ...
        {sections.length === 0 && (
          ...
          <TabsContent value="breakdown" className="mt-0">
            <TaskBreakdownView workspaceId={workspaceId} />
          </TabsContent>
        </Tabs>
      </div>
...
function TaskBreakdownView({ workspaceId }: { workspaceId: string }) {
  const { data: breakdown, isLoading: breakdownLoading, error: breakdownError } = useTaskBreakdown(workspaceId);
  const { data: lifetime, isLoading: lifetimeLoading, error: lifetimeError } = useLifetimeTaskStats();
  const { data: dealTeamContacts = [] } = useDealTeamContacts(workspaceId);
  const [expandedUser, setExpandedUser] = useState<string | null>(null);
  const [showLifetime, setShowLifetime] = useState(false);
  const [expandedLifetimeUser, setExpandedLifetimeUser] = useState<string | null>(null);

  const findName = (id: string) =>
    dealTeamContacts.find(c => c.id === id)?.displayName || id?.replace(/^contact_|_.*$/g, '') || 'Unknown';
  ...
  if (breakdownLoading) return <Skeleton className="h-48 w-full" />;
  if (breakdownError) return (
    <div className="flex items-center gap-2 p-4 text-red-600 bg-red-50 dark:bg-red-900/20 rounded-lg">
      ...
      <span className="text-sm">Failed to load task breakdown. Please try again.</span>
    </div>
  );

  const byUser = breakdown?.byUser || [];
  const unassigned = breakdown?.unassigned || [];
  const lifetimeUsers = lifetime?.byUser || [];

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold">Tasks by Assigned To</h3>
          <p className="text-xs text-muted-foreground">
            {byUser.length} team member{byUser.length !== 1 ? 's' : ''} with assignments · {unassigned.length} unassigned
          </p>
        </div>
        <Button
        ...
        >
          ...
          {showLifetime ? 'Hide' : 'Show'} Lifetime Stats
        </Button>
      </div>
      ...
      {byUser.length === 0 && unassigned.length === 0 && (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-sm">No tasks assigned yet. Assign tasks to team members to see breakdown.</p>
        </div>
      )}

      {byUser.map(user => {
        const isExpanded = expandedUser === user.memberId;
        const completionPct = user.totalCount > 0 ? Math.round((user.completedCount / user.totalCount) * 100) : 0;
        const name = findName(user.memberId);

        return (
          <div key={user.memberId} className="border rounded-xl overflow-hidden">
            <div
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary" className="text-[10px] px-1.5 py-0">{user.totalCount} tasks</Badge>
                  {user.overdueCount > 0 && (
                    <Badge variant="destructive" className="text-[10px] px-1.5 py-0">
                      <AlertCircle className="h-2.5 w-2.5 mr-0.5" />{user.overdueCount} overdue
                    </Badge>
                  )}
                </div>
              ...
              <div className="flex items-center gap-4 text-center">
                <div>
                  <div className="text-lg font-bold text-green-600">{user.completedCount}</div>
                  <div className="text-[10px] text-muted-foreground">Done</div>
                </div>
                <div>
                  <div className="text-lg font-bold">{user.totalCount - user.completedCount}</div>
              ...
              <div className="border-t bg-muted/20 p-3">
                <div className="space-y-1.5">
                  {user.tasks.map(task => {
                    ...
                    return (
                      <div key={task.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-card border text-sm">
                        <Badge variant="outline" className={`text-[9px] px-1 py-0 ${pc.className}`}>{pc.label}</Badge>
                        <span className={`flex-1 truncate ${task.isOverdue ? 'text-red-600 dark:text-red-400' : ''}`}>{task.title}</span>
                        <span className="text-[10px] text-muted-foreground">{task.section}</span>
                        {task.dueDate && (
                          <span className={`text-[10px] ${task.isOverdue ? 'text-red-600 font-medium' : 'text-muted-foreground'}`}>
                            {format(new Date(task.dueDate), 'MMM d')}
                          </span>
                        )}
                        <Badge variant="outline" className={`text-[9px] px-1 py-0 ${sc.bg} ${sc.text}`}>{sc.label}</Badge>
                      </div>
                    );
                  })}
                </div>
      ...
      {unassigned.length > 0 && (
        <div className="border rounded-xl overflow-hidden border-dashed">
          <div className="flex items-center gap-3 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Badge variant="secondary" className="text-[10px] px-1.5 py-0 ml-2">{unassigned.length} tasks</Badge>
            </div>
          </div>
        ...
        <div className="mt-8 space-y-4">
          <div className="border-t pt-6">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-sm font-semibold">Lifetime Stats</h3>
            </div>
            <p className="text-xs text-muted-foreground mb-4">
              Across {lifetime?.projectCount || 0} project{(lifetime?.projectCount || 0) !== 1 ? 's' : ''} in your organization
            </p>
          </div>

          {lifetimeLoading && <Skeleton className="h-32 w-full" />}

          {lifetimeError && (
            <div className="flex items-center gap-2 p-4 text-red-600 bg-red-50 dark:bg-red-900/20 rounded-lg">
              <AlertCircle className="h-4 w-4" />
              <span className="text-sm">Failed to load lifetime stats.</span>
            </div>
          )}

          {!lifetimeLoading && !lifetimeError && lifetimeUsers.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-8">No lifetime data available yet.</p>
          )}

          {lifetimeUsers.map(user => {
            const isExpanded = expandedLifetimeUser === user.memberId;
            const name = user.displayName || findName(user.memberId);

            return (
              <div key={user.memberId} className="border rounded-xl overflow-hidden">
                <div
                  className="flex items-center gap-3 p-4 cursor-pointer hover:bg-muted/50 transition-colors"
                  onClick={() => setExpandedLifetimeUser(isExpanded ? null : user.memberId)}
                >
                  <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                    <span className="text-xs font-bold text-primary">{getInitials(name)}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden max-w-[200px]">
                        <div
                          className={`h-full rounded-full transition-all ${user.completionRate >= 80 ? 'bg-green-500' : user.completionRate >= 50 ? 'bg-amber-500' : 'bg-blue-500'}`}
                          style={{ width: `${user.completionRate}%` }}
                        />
                      </div>
                      <span className="text-xs text-muted-foreground">{user.completionRate}% lifetime</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 text-center">
                    <div>
                      <div className="text-lg font-bold">{user.totalTasks}</div>
                    ...
                    {user.overdueTasks > 0 && (
                      <div>
                        <div className="text-lg font-bold text-red-600">{user.overdueTasks}</div>
                ...
                {isExpanded && user.projectBreakdown.length > 0 && (
                  <div className="border-t bg-muted/20 p-3">
                    <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide mb-2 px-1">By Project</div>
                    <div className="space-y-1.5">
                      {user.projectBreakdown.map(pb => {
                        const pct = pb.total > 0 ? Math.round((pb.completed / pb.total) * 100) : 0;
                        return (
                          <div key={pb.workspaceId} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-card border">
                            <span className="text-sm font-medium flex-1 truncate">{pb.name}</span>
                            <div className="flex items-center gap-2">
                              <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full ${pct >= 80 ? 'bg-green-500' : pct >= 50 ? 'bg-amber-500' : 'bg-blue-500'}`}
                                  style={{ width: `${pct}%` }}
                                />
                              </div>
                              <span className="text-[10px] text-muted-foreground w-8 text-right">{pct}%</span>
                            </div>
                            <span className="text-xs text-muted-foreground">{pb.completed}/{pb.total}</span>
                            {pb.overdue > 0 && (
