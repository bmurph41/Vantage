...
import { 
  ...
  FolderKanban, Briefcase, ListTodo, ClipboardList, Plus
} from "lucide-react";
...
// CRM Entities Navigation
const crmEntitiesNav = [
...
// CRM Automation Navigation
const crmAutomationNav = [
...
// CRM Reporting Navigation
const crmReportingNav = [
...
export default function UnifiedSidebar() {
  ...
  const [crmCoreExpanded, setCrmCoreExpanded] = useState(true);
  const [crmEntitiesExpanded, setCrmEntitiesExpanded] = useState(false);
  const [crmToolsExpanded, setCrmToolsExpanded] = useState(false);
  const [crmAutomationExpanded, setCrmAutomationExpanded] = useState(false);
  const [crmReportsExpanded, setCrmReportsExpanded] = useState(false);
  ...
  return (
    <div className="w-64 bg-white shadow-lg flex-shrink-0 flex flex-col h-screen" data-testid="unified-sidebar">
      ...
      <div className="p-4 border-b border-gray-200 flex-shrink-0">
        <div className="flex items-center space-x-2.5 mb-1" data-testid="sidebar-logo">
          ...
          <h1 className="text-lg font-bold text-gray-900 truncate">Marina Platform</h1>
        </div>
        <p className="text-xs text-gray-500 pl-10.5">DD Tracker & CRM</p>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        ...
        <div className="mb-2">
          <SectionHeader 
            title="CRM Core" 
            expanded={crmCoreExpanded} 
            onToggle={() => setCrmCoreExpanded(!crmCoreExpanded)} 
          />
          {crmCoreExpanded && crmCoreNav.map((item) => (
            <NavLink key={item.name} item={item} />
          ))}
        </div>
        
        {/* CRM Entities Section */}
        <div className="mb-2">
          <SectionHeader 
            title="CRM Entities" 
            expanded={crmEntitiesExpanded} 
            onToggle={() => setCrmEntitiesExpanded(!crmEntitiesExpanded)} 
          />
          {crmEntitiesExpanded && crmEntitiesNav.map((item) => (
            <NavLink key={item.name} item={item} />
          ))}
        </div>
        ...
        <div className="mb-2">
          <SectionHeader 
            title="CRM Tools" 
            expanded={crmToolsExpanded} 
            onToggle={() => setCrmToolsExpanded(!crmToolsExpanded)} 
          />
        ...
        <div className="mb-2">
          <SectionHeader 
            title="Automation" 
            expanded={crmAutomationExpanded} 
            onToggle={() => setCrmAutomationExpanded(!crmAutomationExpanded)} 
          />
          {crmAutomationExpanded && crmAutomationNav.map((item) => (
            <NavLink key={item.name} item={item} />
          ))}
        </div>
        ...
        <div className="mb-2">
          <SectionHeader 
            title="CRM Reports" 
            expanded={crmReportsExpanded} 
            onToggle={() => setCrmReportsExpanded(!crmReportsExpanded)} 
          />
          {crmReportsExpanded && crmReportingNav.map((item) => (
            <NavLink key={item.name} item={item} />
          ))}
        </div>
      </nav>
