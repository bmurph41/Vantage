...
import Detail from "@/pages/analysis/rate-comps/Detail";
import { useQuery, useMutation } from "@tanstack/react-query";
import BulkEdit from "@/pages/analysis/rate-comps/BulkEdit";
...
// Type for the rate comps query response
...
export default function CompsDataGrid({
  ...
  const [selectedCompForProject, setSelectedCompForProject] = useState<RateComp | null>(null);
  const [editingComp, setEditingComp] = useState<RateComp | null>(null);
  const [showEditDialog, setShowEditDialog] = useState(false);
  const [expandedPortfolios, setExpandedPortfolios] = useState<Set<string>>(new Set());
  const [showAddPropertyDialog, setShowAddPropertyDialog] = useState(false);
  const [selectedPortfolioForProperty, setSelectedPortfolioForProperty] = useState<RateComp | null>(null);
  ...
  const getPortfolioStatus = (comp: RateComp) => {
  ...
  const portfolioMutation = useMutation<
    { comp: RateComp; isPortfolio: boolean },
    Error,
    { comp: RateComp; isPortfolio: boolean }
  >({
              ...
              try {
                ...
                await Promise.all(
                  childProperties.map(child => 
                    apiRequest('PATCH', `/api/rate-comps/${child.id}`, {
    ...
    onMutate: ({ comp, isPortfolio }) => {
      ...
      const previousData = queryClient.getQueryData<SalesCompsQuery>(['/api/rate-comps']);
      
      // INSTANT UPDATE: Synchronously update the cached query data for immediate visual changes
      queryClient.setQueryData<SalesCompsQuery>(['/api/rate-comps'], (oldData) => {
        if (!oldData?.comps) return oldData;
        
        return {
          ...oldData,
          comps: oldData.comps.map((item: RateComp) => {
            if (item.id === comp.id) {
              return {
                ...item,
                isPortfolio,
      ...
      queryClient.cancelQueries({ queryKey: ['/api/rate-comps'] });
    ...
    onSuccess: (data: { comp: RateComp; isPortfolio: boolean }, variables, context) => {
    ...
    onError: (error: any, { comp, isPortfolio }: { comp: RateComp; isPortfolio: boolean }, context: any) => {
      
      // Restore previous data if available
      if (context?.previousData) {
        queryClient.setQueryData(['/api/rate-comps'], context.previousData);
      }
  ...
  const togglePortfolioStatus = (comp: RateComp, isPortfolio: boolean) => {
    ...
    try {
      ...
      await Promise.all(
        selectedMarinaIds.map(marinaId =>
          apiRequest('PATCH', `/api/rate-comps/${marinaId}`, {
  ...
  const organizeHierarchicalData = (comps: RateComp[]) => {
    const portfolios = comps.filter(comp => comp.isPortfolio);
    const nonPortfolios = comps.filter(comp => !comp.isPortfolio);
    const hierarchicalData: (RateComp & { level: number; isChild?: boolean; isEmptyPortfolioActions?: boolean })[] = [];
  ...
  const getPortfolioNOI = (portfolioComp: RateComp): number | null => {
  ...
  const formatCellValue = (comp: RateComp, column: string) => {
    const value = comp[column as keyof RateComp];
    ...
    switch (column) {
      ...
      case 'profitCenterHospitality': {
        ...
        const typeField = `${column}Type` as keyof RateComp;
    ...
    <>
      ...
      <div className="flex-1 border-2 border-border rounded-lg bg-card shadow-lg overflow-y-hidden" style={{height: 'calc(100vh - 120px)'}}>
        ...
        >
            ...
            <TableHeader className="bg-gray-100 border-b-2 border-gray-300 sticky top-0 z-20">
              <TableRow>
                <TableHead className="data-table-header w-8 bg-gray-100 sticky top-0 z-20 shadow-sm border-b-2 border-gray-300">
                  <Checkbox
                ...
                {columns.map((column, index) => (
                  <TableHead 
                    key={column.key}
                    className={`data-table-header text-left relative select-none transition-all duration-75 bg-gray-100 sticky top-0 z-20 shadow-sm border-b-2 border-gray-300 ${
                      ...
                      <div className="flex items-center flex-1 min-w-0 cursor-grab active:cursor-grabbing">
                        {column.sortable ? (
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-auto p-0 font-semibold text-gray-700 hover:text-gray-900 truncate cursor-pointer"
                            onClick={(e) => {
                              e.stopPropagation();
                              onSort(column.key);
                            }}
                          ...
                          >
                            ...
                            {sortBy === column.key && (
                              sortDir === 'asc' ? 
                              <ChevronUp className="ml-1 h-3 w-3 text-gray-700 flex-shrink-0" /> :
                              <ChevronDown className="ml-1 h-3 w-3 text-gray-700 flex-shrink-0" />
                            )}
                          </Button>
                        ) : (
...
const AddPropertyToPortfolioDialog = ({ 
  ...
  const { data: allCompsResponse, isLoading } = useQuery<SalesCompsQuery>({ 
    queryKey: ['/api/rate-comps?pageSize=10000&page=1'] // Use default fetcher with large page size to get all comps
  });
  ...
  const availableProperties = (allCompsResponse?.comps || []).filter((comp: RateComp) => 
    !comp.isPortfolio && 
  ...
  const filteredProperties = availableProperties.filter((property: RateComp) =>
        ...
        <div className="overflow-y-auto max-h-96">
          {isLoading ? (
            ...
            <div className="space-y-2">
              {filteredProperties.map((property: RateComp) => (
                <div 
                  key={property.id}
