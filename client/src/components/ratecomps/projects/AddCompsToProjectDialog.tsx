...
import type { FilterParams } from '@/lib/ratecomps/api';
...
export default function AddCompsToProjectDialog({
  ...
  const [data, setData] = useState<RateComp[]>([]);
  ...
  const { data: initialData, isLoading: compsLoading } = useQuery({
    ...
    queryFn: () => rateCompsApi.getComps({ ...queryParams, page: 1 }),
    retry: false,
    enabled: open,
  });
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const moreData = await rateCompsApi.getComps({ 
        ...queryParams, 
        page: nextPage 
      });
  ...
  const formatCellValue = (comp: RateComp, field: string) => {
    const value = comp[field as keyof RateComp];
      ...
      <DialogContent className="max-w-7xl max-h-[90vh] overflow-hidden flex flex-col" data-testid="dialog-add-comps-to-project">
        <DialogHeader className="border-b border-border pb-4">
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle>Add Rate Comps to Project</DialogTitle>
              <p className="text-sm text-muted-foreground mt-1">
                Add rate comps to <span className="font-medium">{projectName}</span>
              </p>
            </div>
            <Button
              variant="ghost"
