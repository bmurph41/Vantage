...
import { useProjects, useCreateProject, useUpdateProject, useDeleteProject } from '@/hooks/ratecomps/useProjects';
import { useAuth } from "@/hooks/useAuth";
import { useToast } from "@/hooks/use-toast";
import type { ProjectWithStats } from '@/lib/ratecomps/api';
import type { RcProject, InsertRcProject, UpdateRcProject, User } from "@shared/schema";

interface ProjectListProps {
  onSelectProject?: (project: RcProject) => void;
...
export default function ProjectList({ onSelectProject, selectedProjectId }: ProjectListProps) {
  ...
  const [newProjectWithRecs, setNewProjectWithRecs] = useState<{ project: RcProject; recommendations: any } | null>(null);

  const { data: projects, isLoading, error } = useProjects();
  
  const handleProjectCreated = (project: RcProject, recommendations: any) => {
  ...
  const handleCreateProject = (data: InsertRcProject) => {
  ...
  const handleUpdateProject = (data: UpdateRcProject) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Organize and manage your rate comparison projects
            </p>
          </div>
          {canCreate && (
            <Button 
      ...
      <ProjectForm
        ...
        onSubmit={(editingProject ? handleUpdateProject : handleCreateProject) as ((data: InsertRcProject | UpdateRcProject) => void)}
