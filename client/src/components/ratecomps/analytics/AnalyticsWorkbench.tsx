...
export default function AnalyticsWorkbench() {
  ...
  const { data: columnValues } = useQuery({
    ...
    queryFn: async () => {
      const [statesRes, waterTypesRes, profitCentersRes] = await Promise.all([
        ...
        fetch('/api/rate-comps/column-values/waterType').then(r => r.json()),
        fetch('/api/rate-comps/column-values/profitCenters').then(r => r.json()),
      ]);
      return {
  ...
  const { data: analyticsData, isLoading, refetch } = useQuery<AnalyticsResponse>({
    ...
    queryFn: async () => {
      const response = await apiRequest('/api/rate-comps/analytics', {
        method: 'POST',
