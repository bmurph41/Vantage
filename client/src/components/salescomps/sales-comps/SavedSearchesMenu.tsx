...
import { savedSearchesApi } from '@/lib/salescomps/api';
import { queryClient } from "@/lib/queryClient";
import { useToast } from "@/hooks/use-toast";
import type { FilterState } from '@/lib/salescomps/types';
import type { SavedSearch, InsertSavedSearch, UpdateSavedSearch } from "@shared/schema";
...
export default function SavedSearchesMenu({
  ...
  const createMutation = useMutation({
    mutationFn: (data: InsertSavedSearch) => savedSearchesApi.createSavedSearch(data),
    onSuccess: (newSearch) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: UpdateSavedSearch }) =>
  ...
  const handleOpenDelete = (search: SavedSearch) => {
    setSelectedSearch(search);
...
interface SearchMenuItemProps {
  ...
  onTogglePin: (search: SavedSearch) => void;
}
