...
import type { DocketDeal } from "@shared/schema";
import { format } from "date-fns";
import { PageTour } from "@/components/onboarding/PageTour";
import { TOUR_IDS, docketTourSteps } from "@/lib/tour-configs";

export default function DocketPage() {
  const reportRef = useRef<HTMLDivElement>(null);
  const [selectedOrigin, setSelectedOrigin] = useState<'all' | 'marinaMatch' | 'aiExtraction'>('marinaMatch');

  const { data, isLoading } = useQuery<{ deals: DocketDeal[]; total: number }>({
    queryKey: selectedOrigin === 'all' 
      ? ['/api/docket/deals'] 
      : ['/api/docket/deals', { origin: selectedOrigin }],
  });
  ...
  return (
    <div className="min-h-screen bg-background" ref={reportRef}>
      <PageTour tourId={TOUR_IDS.DOCKET} steps={docketTourSteps} />
      {/* Header */}
      <div className="border-b bg-card">
        <div className="container mx-auto px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold tracking-tight" data-testid="heading-docket">
            ...
            <div className="flex items-center gap-2">
              <ExportPdfButton contentRef={reportRef} filename="docket-ma-spotlight" title="Docket M&A Spotlight" />
        ...
        <Tabs value={selectedOrigin} onValueChange={(v) => setSelectedOrigin(v as any)}>
          ...
          <TabsContent value={selectedOrigin} className="mt-0">
            {isLoading ? (
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3" data-testid="grid-deals" data-tour="docket-feed">
                {deals.map((deal) => (
                  <Card 
                    key={deal.id} 
