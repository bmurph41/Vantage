...
export function ArticleFeedback({ articleId, variant = "dropdown" }: ArticleFeedbackProps) {
  ...
  const { data: articleTags = [] } = useQuery<ArticleTagAssignment[]>({
    queryKey: ["/api/docktalk/articles", articleId, "tags"],
    queryFn: async () => {
      const res = await fetch(`/api/docktalk/articles/${articleId}/tags`, {
        credentials: "include",
      });
      if (!res.ok) return [];
      return res.json();
  ...
  const { data: articleFeedback } = useQuery<{ feedbackType: string }[]>({
    queryKey: ["/api/docktalk/articles", articleId, "feedback"],
    queryFn: async () => {
      const res = await fetch(`/api/docktalk/articles/${articleId}/feedback`, {
        credentials: "include",
      });
      if (!res.ok) return [];
      return res.json();
  ...
  const assignTagMutation = useMutation({
    ...
    onSuccess: () => {
      dockTalkQueryClient.invalidateQueries({ queryKey: ["/api/docktalk/articles", articleId, "tags"] });
  ...
  const removeTagMutation = useMutation({
    mutationFn: async (tagId: number) => {
      const res = await fetch(`/api/docktalk/articles/${articleId}/tags/${tagId}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) {
  ...
  const submitFeedbackMutation = useMutation({
    ...
    onSuccess: (_, { feedbackType }) => {
      dockTalkQueryClient.invalidateQueries({ queryKey: ["/api/docktalk/articles", articleId, "feedback"] });
      dockTalkQueryClient.invalidateQueries({ queryKey: ["/api/docktalk/feedback/stats"] });
