...
const SYSTEM_PROMPT = `You are an expert commercial real estate analyst and copywriter specializing in creating professional Offering Memorandums (OMs). Your writing is:
...
function buildExecutiveSummaryPrompt(context: PropertyContext): string {
  ...
  return `Write a compelling executive summary paragraph (150-200 words) for a commercial real estate Offering Memorandum.${contextStr}

The executive summary should:
1. Open with the investment opportunity
2. Highlight key property attributes
...
function buildInvestmentHighlightsPrompt(context: PropertyContext): string {
  ...
  return `Generate 5-7 compelling investment highlights as bullet points for a commercial real estate Offering Memorandum.${contextStr}
...
function buildMarketCommentaryPrompt(context: MarketContext): string {
  const parts = [];
  if (context.location) parts.push(`Location: ${context.location}`);
  if (context.medianRent) parts.push(`Median Rent: $${context.medianRent.toLocaleString()}`);
  ...
  return `Write a market overview section (150-200 words) for a commercial real estate Offering Memorandum.${contextStr}

The market commentary should:
1. Describe the local/regional market conditions
2. Highlight positive economic indicators
3. Discuss supply/demand dynamics if data is available
...
function buildFinancialAnalysisPrompt(context: PropertyContext): string {
  ...
  return `Write a financial analysis narrative (100-150 words) for a commercial real estate Offering Memorandum.${contextStr}

The analysis should:
1. Summarize the investment metrics
2. Explain the income stability
...
function buildPropertyDescriptionPrompt(context: PropertyContext): string {
  ...
  return `Write a property description (150-200 words) for a commercial real estate Offering Memorandum.${contextStr}

The description should:
1. Describe the physical attributes and condition
2. Highlight key features and amenities
3. Discuss accessibility and visibility
  ...
  switch (request.type) {
    ...
    case 'custom':
      if (!request.customPrompt || request.customPrompt.trim().length === 0) {
        userPrompt = 'Write a professional paragraph for a commercial real estate offering memorandum describing an investment opportunity.';
      } else {
        userPrompt = request.customPrompt;
      }
      break;
    default:
      userPrompt = 'Write a professional executive summary paragraph for a commercial real estate offering memorandum.';
  }
...
export async function improveContent(content: string, instruction: string): Promise<string> {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: SYSTEM_PROMPT },
      { role: "user", content: `Improve the following content for a commercial real estate Offering Memorandum.
...
export async function suggestLayout(contentDescription: string): Promise<{
  ...
  const response = await openai.chat.completions.create({
    model: "gpt-4o",
    messages: [
      { role: "system", content: "You are an expert at designing professional document layouts for commercial real estate Offering Memorandums." },
