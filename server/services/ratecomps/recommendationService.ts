...
import type { RateComp, RcProjectProfile, WeightOverrides, TenantPreferences, RcOrgPreferences } from "@shared/schema";
import { 
...
export class RecommendationService {
  ...
  private async getEffectiveWeights(
    ...
    const tenantPrefs = await this.storage.getRcOrgPreferences(orgId, segmentKey);
   ...
   */
  private calculateCapacityScore(comp: RateComp, targetCapacity?: number): number {
   ...
   */
  private calculateFinancialScore(comp: RateComp, profile: RcProjectProfile): number {
    let score = 0;
    let components = 0;
   ...
   */
  private calculateProfitCentersScore(comp: RateComp, profile: RcProjectProfile): number {
   ...
   */
  private calculateRegionalScore(comp: RateComp, profile: RcProjectProfile): number {
    let score = 0;
    let components = 0;
   ...
   */
  private calculateGeoScore(comp: RateComp, profile: RcProjectProfile): number {
  ...
  async getRecommendations(params: {
    orgId: string;
    projectProfile: RcProjectProfile;
    ...
    const comps = await this.storage.getRateCompsForRecommendation({
      orgId,
      filters: {
        // Geographic filters
  ...
  async updateLearningWeights(params: {
    ...
    await this.storage.upsertRcOrgPreferences({
      orgId,
      segmentKey,
      weights: newWeights,
    });
