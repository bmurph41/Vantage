...
import { aiContentGenerationService } from './ai-content-service';
import {
...
class DocumentBuilderService {
  ...
  async autoGenerateDocument(params: {
    dealId?: string;
    documentType: DocumentType;
    title: string;
    audience?: string;
    ...
    const context = params.dealId
      ? await dataBindingService.getBindingContext(params.dealId)
      : {};
    ...
    for (const section of sections) {
      ...
      const allBindingRequirements = [
        ...sectionDef.requiredDataBindings,
        ...sectionDef.optionalDataBindings,
      ];

      const resolvedBindings = allBindingRequirements.length > 0
        ? await dataBindingService.resolveBindings(allBindingRequirements, context)
        : [];

      const content: Record<string, any> = { ...(section.content as any || {}) };
      for (const binding of resolvedBindings) {
        if (binding.value !== null && binding.value !== undefined) {
          content[binding.bindingKey] = binding.value;
        }
      }
      ...
      await db
        ...
        .where(eq(omDocumentSections.id, section.id));

      if (
        params.enableAI &&
        sectionDef.aiPromptTemplates.length > 0 &&
        aiContentGenerationService.getAvailableProviders().length > 0
      ) {
        try {
          const template = sectionDef.aiPromptTemplates[0];
          const aiContext: Record<string, any> = {};
          for (const binding of resolvedBindings) {
            if (binding.value !== null && binding.value !== undefined) {
              aiContext[binding.bindingKey] = binding.value;
            }
          }

          const aiResult = await aiContentGenerationService.generateSectionContent(
            section.sectionKey,
            template.key,
            aiContext,
          );
          ...
          if (aiResult.content) {
            ...
            if (template.outputFormat === 'bullets') {
              aiContent.bullets = aiResult.content.split('\n').filter((l: string) => l.trim());
            } else {
              aiContent.narrative = aiResult.content;
            }
          ...
          console.error(`AI generation failed for section ${section.sectionKey}:`, error);
        }
      }

      await this.updateSectionCompletion(section.id);
    }

    await this.updateDocument(documentId, { status: 'in_progress' });
    await this.updateDocumentCompletion(documentId);
