...
export class BillingService {
  ...
  /**
   * Check whether a billing payment event (charge or credit/refund) has been
   * processed for this subscription since `sinceInvoiceId` was the latest
   * invoice.  Returns `{ paymentProcessed, paymentCheckFailed }`.
   *
   * - `paymentProcessed`  – true when a new paid invoice or recent credit note
   *   was found; callers should block undo in this state.
   * - `paymentCheckFailed` – true when the Stripe check itself threw; callers
   *   should treat this as fail-closed (undo unavailable) to avoid billing
   *   inconsistencies.
   */
  private async detectPaymentEvent(
    stripeSubscriptionId: string,
    stripeCustomerId: string,
    sinceInvoiceId: string | null,
  ): Promise<{ paymentProcessed: boolean; paymentCheckFailed: boolean }> {
    try {
      const updatedSub = await stripe!.subscriptions.retrieve(
        stripeSubscriptionId,
        { expand: ['latest_invoice'] }
      );
      const latestInvoice = updatedSub.latest_invoice as Stripe.Invoice | null;
      const postInvoiceId = latestInvoice?.id ?? null;

      // A new invoice appeared (id changed vs the pre-change baseline) and was
      // immediately paid.  Only valid when sinceInvoiceId is known — without a
      // baseline we cannot distinguish a new invoice from the pre-existing one.
      const newInvoicePaid =
        sinceInvoiceId !== null &&
        postInvoiceId !== null &&
        postInvoiceId !== sinceInvoiceId &&
        latestInvoice?.status === 'paid' &&
        (latestInvoice?.amount_paid ?? 0) > 0;

      // A credit note was issued for this customer in the last 15 minutes
      // (indicates a refund/credit processed against the downgraded tier).
      const recentCutoff = Math.floor(Date.now() / 1000) - 900; // 15 min
      const creditNotes = await stripe!.creditNotes.list({
        customer: stripeCustomerId,
        limit: 5,
      });
      const creditNoteIssued = creditNotes.data.some(
        (cn) => cn.created >= recentCutoff
      );

      return { paymentProcessed: newInvoicePaid || creditNoteIssued, paymentCheckFailed: false };
    } catch {
      // Stripe read failed — fail-closed so undo cannot bypass a real payment.
      return { paymentProcessed: false, paymentCheckFailed: true };
    }
  }

  /**
   * Check whether a billing payment event occurred after the downgrade.
   * Used on the undo path to close the race window between toast display and
   * the user clicking Undo.
   *
   * `preChangeInvoiceId` — the `latest_invoice.id` that was captured
   * immediately before the downgrade (returned by `changePlan`). When
   * provided, the invoice-ID delta approach is safe. When absent/null,
   * only the credit-note time-window check is used so we avoid false
   * positives from any pre-existing paid invoice on the account.
   *
   * Returns `{ paymentProcessed, paymentCheckFailed }`.
   */
  async checkRecentPaymentEvent(
    orgId: string,
    preChangeInvoiceId: string | null,
  ): Promise<{ paymentProcessed: boolean; paymentCheckFailed: boolean }> {
    if (!stripe) return { paymentProcessed: false, paymentCheckFailed: false };

    const sub = await this.getSubscription(orgId);
    if (!sub?.stripeSubscriptionId) return { paymentProcessed: false, paymentCheckFailed: false };
    ...
    return this.detectPaymentEvent(sub.stripeSubscriptionId, customerId, preChangeInvoiceId);
  }
  ...
  async changePlan(orgId: string, newTier: string) {
    ...
    let preChangeInvoiceId: string | null = null;

    // Update Stripe if configured
    if (stripe && sub.stripeSubscriptionId && tierDef.priceMonthly !== null) {
      // Capture the current latest_invoice ID before the update so we can
      // detect whether a brand-new invoice was generated by this plan change,
      // and return it to the client so it can be echoed back on undo.
      const stripeSub = await stripe.subscriptions.retrieve(sub.stripeSubscriptionId);
      preChangeInvoiceId =
        typeof stripeSub.latest_invoice === 'string'
          ? stripeSub.latest_invoice
          : (stripeSub.latest_invoice as Stripe.Invoice | null)?.id ?? null;
      ...
      // Detect whether Stripe immediately settled a payment or issued a credit
      // note for the new tier.  Fail-closed: if verification throws, mark
      // paymentCheckFailed so callers block undo to avoid billing inconsistency.
      const result = await this.detectPaymentEvent(
