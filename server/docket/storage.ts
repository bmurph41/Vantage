...
import { docketDeals, crmCompanies, type DocketDeal, type InsertDocketDeal, type CrmCompany } from "@shared/schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  createDocketUserFromMarinaUser(data: { marinaUserId: string; orgId: string; email: string | null; role: string; subscriptionTier: string; isActive: boolean }): Promise<User>;
  ...
  getDealsByEntity(entityId: number, role?: 'buyer' | 'seller'): Promise<DocketDeal[]>;
  ...
  // Deal methods
  createDeal(deal: InsertDocketDeal): Promise<DocketDeal>;
  getDeals(filters: {
  ...
  getDealById(id: number): Promise<DocketDeal | undefined>;
  updateDeal(id: number, updates: Partial<InsertDocketDeal>): Promise<DocketDeal | undefined>;
  getDealsByArticle(articleId: number): Promise<DocketDeal[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getEntityAnalytics(entityId: number): Promise<{
    ...
    const allDeals = await db
      ...
      .where(
        or(
          eq(docketDeals.buyerEntityId, entityId),
          eq(docketDeals.sellerEntityId, entityId)
        )
      );
  ...
  async getDealsByEntity(entityId: number, role?: 'buyer' | 'seller'): Promise<DocketDeal[]> {
    let query = db.select().from(docketDeals);

    if (role === 'buyer') {
      query = query.where(eq(docketDeals.buyerEntityId, entityId)) as any;
    } else if (role === 'seller') {
      query = query.where(eq(docketDeals.sellerEntityId, entityId)) as any;
    } else {
      query = query.where(
        or(
          eq(docketDeals.buyerEntityId, entityId),
    ...
    return query.orderBy(desc(docketDeals.announcedDate));
  }
  ...
  async createDeal(deal: InsertDocketDeal): Promise<DocketDeal> {
  ...
  async getDeals(filters: {
    ...
    if (filters.transactionType) {
      conditions.push(eq(docketDeals.transactionType, filters.transactionType as any));
    }

    if (filters.dealStatus) {
      conditions.push(eq(docketDeals.dealStatus, filters.dealStatus as any));
    }
    ...
    if (filters.fromDate) {
      conditions.push(gte(docketDeals.announcedDate, filters.fromDate));
    }

    if (filters.toDate) {
      conditions.push(lte(docketDeals.announcedDate, filters.toDate));
    }
    ...
    const results = await query
      .orderBy(desc(docketDeals.announcedDate), desc(docketDeals.createdAt))
  ...
  async getDealById(id: number): Promise<DocketDeal | undefined> {
    const [deal] = await db.select().from(docketDeals).where(eq(docketDeals.id, id));
    return deal || undefined;
  }

  async updateDeal(id: number, updates: Partial<InsertDocketDeal>): Promise<DocketDeal | undefined> {
    const [updated] = await db
      .update(docketDeals)
      .set({ ...updates, updatedAt: new Date() })
      .where(eq(docketDeals.id, id))
      .returning();
    return updated || undefined;
  }


  async getDealsByArticle(articleId: number): Promise<DocketDeal[]> {
    const results = await db
      .select()
      .from(docketDeals)
      .where(eq(docketDeals.articleId, articleId))
  ...
  async getDealAnalytics(filters?: {
    ...
    let query = db
      .select({
        ...
        announcedDate: docketDeals.announcedDate,
        articleId: docketDeals.articleId,
        region: sql<string>`COALESCE(${articles.region}::text, 'Unknown')`.as('region'),
      })
      .from(docketDeals)
      .leftJoin(articles, eq(docketDeals.articleId, articles.id));
  ...
  async getOrganizationFeature(orgId: string): Promise<{ id: string; orgId: string; feature: string; tier: string; isActive: boolean; activatedAt: Date; expiresAt: Date | null } | undefined> {
    const [feature] = await db
      ...
      .where(and(
        ...
        eq(organizationFeatures.feature, 'docket')
      ));
  ...
  async createOrganizationFeature(data: { orgId: string; feature?: string; tier?: string; isActive?: boolean }): Promise<{ id: string; orgId: string; feature: string; tier: string; isActive: boolean; activatedAt: Date; expiresAt: Date | null }> {
    const [feature] = await db
      ...
      .values({
        orgId: data.orgId,
        feature: data.feature || 'docket',
        tier: data.tier || 'docket_free',
  ...
  async getDocketUserByMarinaUserId(marinaUserId: string): Promise<User | undefined> {
    const [user] = await db
      .select()
      .from(users)
  ...
  async createDocketUserFromMarinaUser(data: { marinaUserId: string; orgId: string; email: string | null; role: string; subscriptionTier: string; isActive: boolean }): Promise<User> {
    const [user] = await db
      .insert(users)
      .values({
