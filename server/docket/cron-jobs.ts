...
import { processNewArticlesForAlerts, processDailyAlerts, processWeeklyAlerts } from "./services/alert-service";
...
export function setAutoFetchEnabled(enabled: boolean) {
  ...
  console.log(`Docket auto-fetch ${enabled ? 'enabled' : 'disabled'}`);
  return getAutoFetchStatus();
}

export function startDocketCronJobs(storage: IStorage): void {
  docketStorage = storage;
  ...
  // Process daily alerts every 15 minutes (checks per-alert delivery time window)
  ...
  // Process weekly alerts every 15 minutes on Mondays (checks per-alert delivery time window)
...
async function updateSystemStats(newArticles: number): Promise<void> {
  try {
    ...
    const todayArticles = await docketStorage.getArticles(null, {
      fromDate: today,
      limit: 1000
    });

    // Get total articles count
    const allArticles = await docketStorage.getArticles(null, { limit: 1 });
    
    // Calculate average relevance
    const recentArticles = await docketStorage.getArticles(null, {
    ...
    await docketStorage.updateSystemStats({
...
async function cleanupOldArticles(): Promise<void> {
  try {
    ...
    const deletedCount = await docketStorage.deleteOldArticles(oneYearAgo);
