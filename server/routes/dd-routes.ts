...
const authorizeProjectAccess = async (projectId: string, orgId: string) => {
  ...
  if (project.orgId !== orgId) throw new Error("Unauthorized access to project");
  return project;
};
...
const authorizeCalendarEventAccess = async (eventId: string, orgId: string) => {
  ...
  if (!project || project.orgId !== orgId) throw new Error("Unauthorized access to calendar event");
  return event;
};

export function registerDDRoutes(
  app: Express,
