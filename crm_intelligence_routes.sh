#!/bin/bash
# =============================================================
#  MISSING ROUTES: Comments, Playbooks, SLA, Red Flags, Phase Gates
#  All DB tables already exist — just need the endpoints
# =============================================================
set -e

echo "=== Writing server/routes/crm-intelligence-routes.ts ==="
cat > server/routes/crm-intelligence-routes.ts << 'ROUTES'
/**
...
ROUTES
echo "  ✓ crm-intelligence-routes.ts written"

echo ""
echo "=== Register routes in server/routes.ts ==="
node --input-type=module << 'JS'
import { readFileSync, writeFileSync } from 'fs';
const path = 'server/routes.ts';
let src = readFileSync(path, 'utf8');

if (src.includes('crm-intelligence-routes')) {
...
console.log('  ✓ Routes registered: /api/comments, /api/crm (intelligence), /api/sla');
JS

echo ""
echo "=== Verify ==="
grep -c "crm-intelligence-routes" server/routes.ts && echo "  ✓ Registered in routes.ts" || echo "  ✗ Not registered"
wc -l server/routes/crm-intelligence-routes.ts

echo ""
echo "✅ All missing API routes added."
echo ""
echo "Now live:"
echo "  /api/comments/threads         — get/create/pin/resolve threaded discussions"
echo "  /api/comments/threads/:id/comments — add replies with @mentions"
echo "  /api/crm/playbooks            — deal playbook templates & progress"
echo "  /api/crm/deals/:id/playbook-progress — per-deal checklist state"
echo "  /api/crm/red-flags/*          — raise/ack/resolve/dismiss red flags"
echo "  /api/crm/phase-gates/*        — stage transition approvals"
echo "  /api/crm/forecasting/*        — summary/close-rates/stage-analysis/velocity"
echo "  /api/sla/configs              — SLA configuration CRUD"
echo "  /api/sla/tracking             — active SLA tracking"
echo "  /api/sla/summary              — SLA health overview"
echo ""
