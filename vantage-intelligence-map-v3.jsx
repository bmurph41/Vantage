...
// Fields per entry:
//   label       - display name
//   icon        - emoji for map marker + sidebar
//   color       - hex, drives heat map, pills, charts, badges
//   group       - sidebar grouping category
//   sizeLabel   - what "size" means (Slips, Units, Sq Ft, Acres, Bays…)
//   occLabel    - what occupancy means (Occupancy, Leased %, Utilization…)
//   rev         - array of 3 revenue stream labels
//   demandKey   - the asset-specific demand metric label shown in Demographics
//   priceUnit   - denominator for price-per-unit calc (e.g. "Slip", "Unit", "Sq Ft")
//   radarLabels - 6 labels for the performance radar (defaults if omitted)
// ═══════════════════════════════════════════════════════════════════════════════
...
// TIER + STATUS COLORS
// ═══════════════════════════════════════════════════════════════════════════════
const TIER_COLOR   = { "A+":"#00f5d4", A:"#4ade80", B:"#facc15", C:"#f87171", D:"#f87171" };
const STATUS_COLOR = { "Active":"#4ade80","Stabilized":"#00f5d4","Value-Add":"#facc15","Distressed":"#f87171","Under Contract":"#a78bfa","Off Market":"#94a3b8","Prospect":"#60a5fa" };

// ═══════════════════════════════════════════════════════════════════════════════
// SAMPLE PROPERTIES  (each property only needs to match a key in ASSET_REGISTRY)
// ═══════════════════════════════════════════════════════════════════════════════
...
function Pill({ label, color="#4a7fa5" }) {
...
function BarGauge({ label, pct, color }) {
  return (
...
// MAP CANVAS
// ═══════════════════════════════════════════════════════════════════════════════
function MapCanvas({ properties, selected, onSelect }) {
...
function DetailPanel({ p, onClose }) {
  ...
  // Pull everything from registry — fallback gracefully if new class not yet configured
  const ac     = ASSET_REGISTRY[p.assetClass] || { label:p.assetClass, icon:"🏢", color:"#4a7fa5", group:"Other", sizeLabel:"Units", occLabel:"Occ %", rev:["Revenue 1","Revenue 2","Revenue 3"], demandKey:"Demand Index", priceUnit:"Unit" };
  const tierC  = TIER_COLOR[p.tier]  || "#4a7fa5";
  ...
  const sizeFormatted = p.size >= 10000
    ? `${(p.size/1000).toFixed(0)}K`
    : p.size.toLocaleString();

  const pricePerUnit = p.size > 0
    ? `$${Math.round(p.askingPrice/p.size).toLocaleString()}`
    : "—";
        ...
        {tab==="overview" && (
          <div style={{ display:"flex",flexDirection:"column",gap:9 }}>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6 }}>
              ...
              <StatBox label="Year Built"   value={p.yearBuilt} sub={`${2025-p.yearBuilt}yr`} />
            </div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
              <StatBox label="Acreage" value={`${p.acreage} ac`} />
              {p.waterFrontage>0 && <StatBox label="Water Frontage" value={`${p.waterFrontage} ft`} accent={acColor} />}
            </div>
            ...
            <div style={{ background:"#07111c",borderRadius:5,padding:"9px 11px",border:"1px solid #102030" }}>
              ...
              <ResponsiveContainer width="100%" height={115}>
                <BarChart data={[{name:ac.rev[0].split("/")[0],value:p.rev1},{name:ac.rev[1].split("/")[0],value:p.rev2},{name:ac.rev[2].split("/")[0],value:p.rev3}]}>
                  ...
                  <YAxis tick={{fill:"#2a5070",fontSize:7}} tickFormatter={v=>`$${v/1000}K`}/>
                  <Tooltip formatter={v=>[`$${v.toLocaleString()}`,""]} contentStyle={{background:"#050d18",border:"1px solid #102030",borderRadius:4,fontSize:9}}/>
            ...
            <div style={{ background:"#07111c",borderRadius:5,padding:"9px 11px",border:"1px solid #102030" }}>
              ...
              <ResponsiveContainer width="100%" height={115}>
                <LineChart data={rentData}>
                  ...
                  <YAxis tick={{fill:"#2a5070",fontSize:7}} tickFormatter={v=>`${v}%`}/>
                  <Tooltip formatter={v=>[`${v}%`,"Growth"]} contentStyle={{background:"#050d18",border:"1px solid #102030",borderRadius:4,fontSize:9}}/>
            ...
            <div style={{ background:"#07111c",borderRadius:5,padding:"9px 11px",border:"1px solid #102030" }}>
              ...
              <ResponsiveContainer width="100%" height={115}>
                <LineChart data={vacData}>
                  ...
                  <XAxis dataKey="year" tick={{fill:"#2a5070",fontSize:9}}/>
                  <YAxis tick={{fill:"#2a5070",fontSize:7}} tickFormatter={v=>`${v}%`}/>
                  <Tooltip formatter={v=>[`${v}%`,"Vacancy"]} contentStyle={{background:"#050d18",border:"1px solid #102030",borderRadius:4,fontSize:9}}/>
...
export default function VantageIntelligenceMap() {
  ...
  const [activeGroups, setActiveGroups] = useState(()=>new Set(Object.values(ASSET_REGISTRY).map(v=>v.group)));
  const [filterTier,   setFilterTier]   = useState("ALL");
  const [searchText,   setSearchText]   = useState("");
  const [addAddress,   setAddAddress]   = useState("");

  // Derive all unique groups from registry (auto-updates when registry grows)
  ...
  }, [activeClassKeys, filterTier, searchText]);
  ...
  return (
    <div style={{ fontFamily:"'IBM Plex Sans','Helvetica Neue',sans-serif",background:"#030810",color:"#c0d8ee",height:"100vh",display:"flex",flexDirection:"column",overflow:"hidden" }}>
      ...
      <div style={{ height:48,padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #081824",background:"#040a14",flexShrink:0 }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:7 }}>
          ...
          <input value={addAddress} onChange={e=>setAddAddress(e.target.value)}
            placeholder="Geocode new address…"
            style={{ background:"#06101c",border:"1px solid #10253a",borderRadius:4,padding:"4px 9px",color:"#c0d8ee",fontSize:9,width:185,outline:"none" }}/>
      ...
      <div style={{ flex:1,display:"flex",overflow:"hidden" }}>
        ...
        <div style={{ width:230,borderRight:"1px solid #081824",background:"#040a14",display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden" }}>
          ...
          <div style={{ padding:"9px 9px 7px",borderBottom:"1px solid #081824",overflowY:"auto",maxHeight:"55%" }}>
            ...
            {allGroups.map(group=>{
              ...
              const presentInFiltered = items.some(([k])=>filtered.some(p=>p.assetClass===k));
              return (
        ...
        <div style={{ flex:1,position:"relative",overflow:"hidden" }}>
          ...
          <div style={{ position:"absolute",bottom:12,left:12,background:"#030810e8",border:"1px solid #081824",borderRadius:6,padding:"8px 11px",backdropFilter:"blur(8px)" }}>
            ...
            {[["A+","90–100"],["A","80–89"],["B","65–79"],["C","below 65"]].map(([t,l])=>(
      ...
      <div style={{ height:24,borderTop:"1px solid #081824",background:"#02060f",display:"flex",alignItems:"center",padding:"0 14px",gap:14,flexShrink:0 }}>
        ...
        <span style={{ fontSize:7,color:"#0e2030" }}>{filtered.length}/{SAMPLE_PROPERTIES.length} Properties Visible</span>
        <span style={{ fontSize:7,color:"#0e2030" }}>·</span>
