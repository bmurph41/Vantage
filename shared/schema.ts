...
export const marinaBudgets = pgTable("marina_budgets", {
  ...
  orgId: varchar("org_id").notNull().references(() => organizations.id),
  ownedAssetId: varchar("owned_asset_id").notNull().references(() => ownedAssets.id, { onDelete: 'cascade' }),
  rentRollId: varchar("rent_roll_id").references(() => rentRolls.id),
  fiscalYear: integer("fiscal_year").notNull(),
  name: text("name"),
  description: text("description"),
  status: text("status").notNull().default("draft"),
  totalBudgetAmount: decimal("total_budget_amount", { precision: 15, scale: 2 }),
  rentRollSyncEnabled: boolean("rent_roll_sync_enabled").default(false),
  lastRentRollSyncAt: timestamp("last_rent_roll_sync_at"),
  createdBy: varchar("created_by").notNull().references(() => users.id),
  ...
  rentRollIdx: index("marina_budgets_rent_roll_idx").on(table.rentRollId),
...
// Rent Roll Snapshots - Monthly aggregated metrics for tracking over time
export const rentRollSnapshots = pgTable("rent_roll_snapshots", {
  ...
  ownedAssetId: varchar("owned_asset_id").references(() => ownedAssets.id),
  ...
  grossPotentialRevenue: decimal("gross_potential_revenue", { precision: 15, scale: 2 }).notNull().default("0"),
  occupiedRevenue: decimal("occupied_revenue", { precision: 15, scale: 2 }).notNull().default("0"),
  vacancyLoss: decimal("vacancy_loss", { precision: 15, scale: 2 }).notNull().default("0"),
  
  averageMonthlyRate: decimal("average_monthly_rate", { precision: 10, scale: 2 }),
  revenuePerUnit: decimal("revenue_per_unit", { precision: 10, scale: 2 }),
  ...
  orgIdx: index("rent_roll_snapshots_org_idx").on(table.orgId),
  rentRollIdx: index("rent_roll_snapshots_rent_roll_idx").on(table.rentRollId),
  assetIdx: index("rent_roll_snapshots_asset_idx").on(table.ownedAssetId),
  periodIdx: index("rent_roll_snapshots_period_idx").on(table.snapshotYear, table.snapshotMonth),
  uniqueRentRollPeriod: unique().on(table.rentRollId, table.snapshotYear, table.snapshotMonth),
}));

// Rent Roll Snapshot Details - Category-level breakdowns within a snapshot
export const rentRollSnapshotDetails = pgTable("rent_roll_snapshot_details", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  snapshotId: varchar("snapshot_id").notNull().references(() => rentRollSnapshots.id, { onDelete: 'cascade' }),
  ...
  grossRevenue: decimal("gross_revenue", { precision: 15, scale: 2 }).notNull().default("0"),
  ...
  snapshotIdx: index("rent_roll_snapshot_details_snapshot_idx").on(table.snapshotId),
  categoryIdx: index("rent_roll_snapshot_details_category_idx").on(table.snapshotId, table.categoryType),
}));

// Budget Rent Roll Bindings - Map budget line items to rent roll categories
export const budgetRentRollBindings = pgTable("budget_rent_roll_bindings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  budgetId: varchar("budget_id").notNull().references(() => marinaBudgets.id, { onDelete: 'cascade' }),
  lineItemId: varchar("line_item_id").notNull().references(() => marinaBudgetLineItems.id, { onDelete: 'cascade' }),
  rentRollId: varchar("rent_roll_id").notNull().references(() => rentRolls.id),
  
  bindingScope: text("binding_scope").notNull().default("overall_storage"),
  ...
  budgetIdx: index("budget_rent_roll_bindings_budget_idx").on(table.budgetId),
  lineItemIdx: index("budget_rent_roll_bindings_line_item_idx").on(table.lineItemId),
  rentRollIdx: index("budget_rent_roll_bindings_rent_roll_idx").on(table.rentRollId),
  uniqueBinding: unique().on(table.lineItemId, table.rentRollId, table.bindingScope),
}));

// Rent Roll Snapshot Relations
export const rentRollSnapshotsRelations = relations(rentRollSnapshots, ({ one, many }) => ({
  organization: one(organizations, {
    fields: [rentRollSnapshots.orgId],
    references: [organizations.id],
  }),
  rentRoll: one(rentRolls, {
    fields: [rentRollSnapshots.rentRollId],
    references: [rentRolls.id],
  }),
  ownedAsset: one(ownedAssets, {
    fields: [rentRollSnapshots.ownedAssetId],
    references: [ownedAssets.id],
  }),
  details: many(rentRollSnapshotDetails),
}));

export const rentRollSnapshotDetailsRelations = relations(rentRollSnapshotDetails, ({ one }) => ({
  snapshot: one(rentRollSnapshots, {
    fields: [rentRollSnapshotDetails.snapshotId],
    references: [rentRollSnapshots.id],
  }),
}));

export const budgetRentRollBindingsRelations = relations(budgetRentRollBindings, ({ one }) => ({
  budget: one(marinaBudgets, {
    fields: [budgetRentRollBindings.budgetId],
    references: [marinaBudgets.id],
  }),
  lineItem: one(marinaBudgetLineItems, {
    fields: [budgetRentRollBindings.lineItemId],
    references: [marinaBudgetLineItems.id],
  }),
  rentRoll: one(rentRolls, {
    fields: [budgetRentRollBindings.rentRollId],
    references: [rentRolls.id],
  }),
}));
...
export const updateRentRollSnapshotSchema = insertRentRollSnapshotSchema.partial();

export const insertRentRollSnapshotDetailSchema = createInsertSchema(rentRollSnapshotDetails).omit({
  id: true,
  createdAt: true,
});

export const insertBudgetRentRollBindingSchema = createInsertSchema(budgetRentRollBindings).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export const updateBudgetRentRollBindingSchema = insertBudgetRentRollBindingSchema.partial();
...
export type UpdateRentRollSnapshot = z.infer<typeof updateRentRollSnapshotSchema>;

export type RentRollSnapshotDetail = typeof rentRollSnapshotDetails.$inferSelect;
export type InsertRentRollSnapshotDetail = z.infer<typeof insertRentRollSnapshotDetailSchema>;

export type BudgetRentRollBinding = typeof budgetRentRollBindings.$inferSelect;
export type InsertBudgetRentRollBinding = z.infer<typeof insertBudgetRentRollBindingSchema>;
export type UpdateBudgetRentRollBinding = z.infer<typeof updateBudgetRentRollBindingSchema>;
