...
- **`doc-intel-service.ts` marina hardcoding outside Task 5 scope** — Phase 0 audit (2026-05-18) surfaced four marina-specific code paths in `server/services/doc-intel-service.ts` beyond the L1760-1797 zone that Task 5 (Phase 1 item #4) addresses: L211/L219 built-in regex patterns (wet slip, dry storage, winter storage); L2401-2441 marina-specific tier-inference fallback (dockage / slip rental keywords); L2528 deferred `inferDeptForActual` assetClass TODO (`assetClass not in scope` comment in code today). These are Path 1 violations per Section 5 principle #1 (asset-class-agnostic architecture) — each represents class-specific logic in shared infrastructure. Should be addressed in Phase 1.5 per-class content audit (§8) as targeted cleanups, not bundled into Task 5 (which is scoped to LLM prompt branching only).
- **LLM provider divergence: OpenAI vs Anthropic across services** — `doc-intel-service.ts` uses OpenAI (`gpt-4o-mini`); `claude-extractor.ts`, `exit-ai-insights-service.ts`, `broker-evaluator-service.ts`, `ai/spending-guard.ts` use Anthropic. No `LlmProvider` abstraction exists. Phase 0 audit (2026-05-18) confirmed this is an intentional-or-accidental divergence worth consolidating, but consolidation requires its own decision pass (cost, latency, output quality on real CRE P&Ls). Deferred to post-MVP; `doc-intel-service.ts` stays on OpenAI until that pass.
- **`pct()` helper ambiguity for value = 1 in direct-input-engine** — the percentage normalization helper at `server/services/direct-input-engine.ts:956` uses `n > 1 ? n / 100 : n`, which treats a value of exactly 1 as 100% instead of 1%. Surfaced 2026-05-18 during MF fixture seed (commit `38421aea`) when `badDebtPct: 1` produced negative EGI and $0 management fee. Worked around in the fixture by omitting `badDebtPct` and relying on engine default (0.01). Real bug: any percentage field configured at exactly 1 hits this trap. Fix candidates: (a) change contract to "always pass as decimal" with caller responsibility, (b) change contract to "always pass as percent (0-100)" with helper always dividing, (c) add explicit boundary check `n >= 1 && n <= 100 ? n / 100 : n` with value > 100 treated as out-of-range. Decision deferred — needs full review of all `pct()` callers to determine which contract minimizes existing data migration risk. Pairs with Phase 1.5 audit (§8) since it's a registry-adjacent helper used across the COA registries.
...
- **`dd_template_asset_class` Postgres enum survived platform-wide varchar migration** — when Vantage migrated 55+ asset-class columns off `pgEnum` to varchar (per the platform-wide asset-class agnosticism work), the `dd_template_asset_class` enum on `dd_checklist_templates.asset_class` was missed. Surfaced 2026-05-19 during Phase 4a Item 5 (commit `c9a63311`) when adding `'str'` to DD templates required an `ALTER TYPE … ADD VALUE` migration (`db/migrations/007_dd_template_asset_class_str.sql`). Every new MVP-or-beyond asset class will hit this same wall. Proper fix: drop the enum, convert column to varchar, update Drizzle schema (`shared/schema.ts:1356`), audit downstream consumers for enum-type comparisons. Workaround in place: add new asset classes to the enum via `ALTER TYPE` migrations as needed. Pairs with the parallel-registries and LLM-provider-divergence patterns above. Recommended for Phase 4a tail or v1.1 housekeeping wave.
- **`POST /api/dd-checklist-templates/seed` omits required `template_type` column** — at `server/routes/dd-checklist-routes.ts:1040`, the seed endpoint's INSERT statement is missing the `template_type` column which has a NOT-NULL constraint on `dd_checklist_templates`. Endpoint has never been successfully invoked — Item 5 (commit `c9a63311`) worked around this by direct DB INSERT with `template_type='custom'` (matching the convention in the `/create-custom` endpoint at line 1079). Surfaced 2026-05-19 during Phase 4a Item 5 verification. Fix: add `template_type='custom'` (or appropriate value) to the INSERT statement. Trivial scope (~30 minutes). Recommended for Phase 4a cleanup wave.
- **`GET /api/dd-checklist-templates?assetClass=X` ignores the `assetClass` query parameter** — the route at `server/routes/dd-checklist-routes.ts` ORs on `isBuiltin` and `orgId` but does not filter by `assetClass` despite accepting it as a query param. Result: friendlies requesting MF-specific DD templates receive all templates including marina, retail, etc. Surfaced 2026-05-19 during Phase 4a Item 5 verification. Fix: add WHERE clause for `assetClass` when the param is present. Trivial scope (~30 minutes). Recommended for Phase 4a cleanup wave.
- **`pro-forma-charts` analytics endpoint is a 100% mock — PRE-BETA BLOCKER, not post-MVP polish** (listed in §3.5 only for proximity to the related Item 7 cleanup; this is NOT ordinary cleanup). `GET /api/analytics/modeling/projects/:projectId/pro-forma-charts` (`server/routes/analytics-routes.ts:1248`) returns entirely hardcoded marina-flavored literals — Wet Slips $2.45M, Fuel Sales $2.85M, Dry Storage $1.125M, plus Ship Store / Ancillary revenue and Payroll / Utilities / Insurance expense breakdowns — yielding a fixed NOI of **$4.37M** ($7.5M revenue − $3.13M expenses) and a fixed 6.8% cap rate. The `projectId` and `year` request params are destructured and echoed back in the response but never used to query anything: the payload is byte-identical for every project and every asset class. This endpoint feeds the workspace "Pro Forma Charts" tab. Surfaced 2026-05-20 during Phase 4a Item 7 Phase 0 — the real Item 7 bug (the NOI Projection chart on the *Pro Forma* tab) was a separate client-side field-name mismatch in `pro-forma.tsx`, fixed in commit `931fbbe3`; the audit had named the wrong file (`pro-forma-charts.tsx`), and tracing that mis-pointer led here. **Why this is a blocker, not polish:** it is strictly worse than the flat-zero chart Item 7 fixed. A flat-zero line at least reads as "no data"; a mock endpoint renders plausible, professional-looking fabricated financials that a beta friendly will mistake for their own deal's projections. Shipping fabricated numbers to friendlies is a credibility hazard. **Fix:** rewrite the endpoint to call `proFormaEngineService` (the engine behind `GET /api/modeling/projects/:projectId/pro-forma`) and derive the chart aggregations from real per-project data. **Fallback if not built before friendlies:** gate or hide the "Pro Forma Charts" tab entirely — a disabled tab is honest; a mock-fed tab is not. Ranked as **Item 7b** in `BETA_MVP_PHASE_1_5_AUDIT.md` §6.A.
- **`GET /api/analytics/rent-roll/interactive-analytics` is a 100% mock — PRE-BETA BLOCKER.** `server/routes/analytics-routes.ts:1079`. Returns hardcoded marina rent-roll analytics (283 slips, 87.5% occupancy, $1.85M revenue) with `Math.random()` jitter; destructures `startDate`/`endDate`/`locationId` but ignores them; identical payload for every org. Feeds the rent-roll-v2 "Interactive Analytics" page. **Fallback (recommended before friendlies — the real fix is ~1-2 days):** gate/hide the Interactive Analytics page until a real rent-roll aggregation service exists; do not wire under time pressure. See `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Finding 1.
- **`POST /api/funds/:fundId/send-report` is a no-op that claims success — PRE-BETA BLOCKER.** `server/routes/modeling-routes.ts:5557`. Returns `{ sent: true }` ("Reports queued for delivery to all fund investors") while sending nothing and ignoring `:fundId`. A GP clicks "Send Report" in Investor Reporting, sees a green confirmation, and LPs receive nothing. **Fallback:** disable the "Send Report" button until wired (~3-6h, lift the `lp-portal-service` delivery path) — a disabled button is honest, a fake success is a credibility hazard. See `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Finding 2.
- **`GET /api/rent-roll/analytics/revenue` fabricates revenue — PRE-BETA BLOCKER.** `server/routes/rra-routes.ts:2319`. Returns 12 months of `Math.random()`-generated revenue (different numbers every page load); ignores `orgId`/`year`; zero DB calls. Feeds the rent-roll Executive Modals. **Fallback:** wire to a real `rraService` revenue query (medium effort), or gate the Executive Modals as a unit with the rent-roll cluster (Findings 3-6). See `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Finding 3.
- **`GET /api/rent-roll/analytics/storage-types` returns hardcoded marina types — PRE-BETA BLOCKER.** `server/routes/rra-routes.ts:2337`. Hardcoded 4-type marina array (Wet Slip 120/$240k, Dry Storage 80/$96k, …); ignores `orgId`; zero DB calls. Feeds the rent-roll Executive Modals; also re-introduces marina vocabulary on an asset-class-agnostic platform. **Fallback:** wire to a real by-storage-type aggregation, or gate with the rent-roll cluster. See `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Finding 4.
- **`GET /api/rent-roll/analytics/lease-terms` returns hardcoded buckets — PRE-BETA BLOCKER.** `server/routes/rra-routes.ts:2352`. Hardcoded 4-bucket lease-term array (Monthly 45/$850, Annual 120/$750, …); ignores `orgId`; zero DB calls. Feeds the rent-roll Executive Modals. **Fallback:** wire to a real lease-term grouping query, or gate with the rent-roll cluster. See `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Finding 5.
- **`/api/rent-roll/custom-types` CRUD is non-persistent — PRE-BETA BLOCKER.** `server/routes/rra-routes.ts:2391`. GET returns 15 hardcoded records (string IDs `'1'`–`'15'`, ignores `orgId`); POST/PATCH/DELETE echo success but never touch the DB — user-created/edited custom types silently vanish on reload. **Fallback:** wire to a real `rra_custom_types` table (medium-large), or gate the Custom Types management UI. See `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Finding 6.
- **`CensusService.getMockDemographics()` silently substitutes fake demographics — PRE-BETA BLOCKER.** `server/services/census-service.ts:1241` (silent catch at `:453`). When `CENSUS_API_KEY` is unset *or the live Census API throws*, returns a complete fabricated demographic profile (population, income, age/income/education/race distributions) with **no `dataSource: 'mock'` flag** — indistinguishable from real Census data, even in production on a transient API failure. Feeds the Demographics analysis pages. **Fallback (recommended — fix is small, ~1-2h):** WIRE, fail loud — return a 503 / `dataSource: 'unavailable'` or stamp `isMockData: true`; the `:453` catch must not silently serve mock data. See `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Finding 7.
- **`LeaseCashFlowEngine.generateSyntheticRentRoll()` fabricates a 153-unit rent roll — PRE-BETA BLOCKER.** `server/services/lease-cashflow-engine.ts:401`. When a modeling project has no rent roll configured, silently generates 153 fake marina units with `Math.random()` rents/tenants ("Tenant 42") that flow into lease cash-flow, rollover schedule, and IRR/NPV outputs with no "synthetic" indication. Feeds the workspace Lease Cash Flow tab. **Fallback (recommended — fix is small, ~1-2h):** WIRE — return an honest "no rent roll configured" empty state instead of fabricating one. See `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Finding 8.
- **Mock-endpoint audit — 6 further MEDIUM/LOW findings (summary).** Beyond the 8 blockers above, the 2026-05-20 mock-endpoint audit confirmed 6 lower-severity mock/no-op endpoints: `PUT /api/bookkeeping/chart-of-accounts/:id` (no-op persist — edits vanish on refetch), the CRM comment-notifications cluster (4 endpoints — feature silently dead despite a populated `crm_notifications` table), `POST /api/budgets/seed-actuals` (fabricated marina actuals reachable from a button in a production Ops › Budgeting tab), `assessAssetRisk()` (4 of 6 risk dimensions hardcoded "placeholder scoring"), `dashboard-service.generateModulePreview()` (hardcoded +12.5% trend), and `POST /api/admin/benchmarks/rebuild` (internal no-op, fabricated `status: 'pending'`). Not individually itemized here to keep §3.5 readable — full detail, locations, friendly-facing surfaces, and fix scopes in `BETA_MVP_MOCK_ENDPOINT_AUDIT.md` Findings 9-14. Recommended for the Phase 4a cleanup wave after the 8 blockers.
...
| 2026-05-19 (later) | Brett + Claude (Opus 4.7 1M) | Captured `dd_template_asset_class` Postgres enum gotcha — surfaced during Phase 4a Item 5 Phase 0 (commit `c9a63311`) when adding `'str'` required an `ALTER TYPE … ADD VALUE` migration. Enum survived the platform-wide varchar migration. Workaround applied (migration `007_*.sql`); proper fix captured in §3.5 cleanup list. |
