"""
Investment Criteria: Match Breakdown + Management Page
=======================================================
1. Add criteria match grid inside the Deal Recommendation card
2. Create investment-criteria.tsx management page

Run from workspace root: python3 apply-criteria-ui.py
"""
import os
...
# ================================================================
# 1. Add criteria match grid to deal-pricing.tsx signal card
# ================================================================
print("=== 1. Add criteria match grid to signal card ===")
DP = "client/src/pages/modeling/projects/workspace/deal-pricing.tsx"
dp = read(DP)

# Find the Key Factors section and add criteria matches after it
# The Key Factors section has dealSignal.reasons.map
# After the reasons, add the criteria match grid

old_key_factors_header = '''                    <p className={cn("text-xs font-semibold uppercase tracking-wider", dealSignal.color)}>Key Factors</p>
                    {dealSignal.reasons.map((reason, i) => ('''

new_key_factors_header = '''                    <p className={cn("text-xs font-semibold uppercase tracking-wider", dealSignal.color)}>Key Factors</p>
                    {dealSignal.reasons.map((reason, i) => ('''

# The criteria grid should go after the reasons section closes
# Let's find the closing of the reasons block and add after
# The reasons are inside a div with rounded-lg p-3 space-y-1.5
# After that div closes, we add criteria matches

# Let's use a different approach - add criteria grid between Key Factors and the card close
# First find the "criteria" flag in the signal card

# Actually the simplest: add after the score bar, before the Key Factors
# No - let's add after Key Factors as a separate section

# Find the end of CardContent in the signal card and add before it
# Unique pattern: the signal card has a specific closing pattern

# Let me just add a criteria match section. The signal IIFE returns JSX.
# I need to add inside the returned JSX of that IIFE.

# Best approach: find the `criteriaMatches` and render them
# The dealSignal now has .criteriaMatches from computeCriteriaSignal

# I'll add after the reasons section. Let me find where reasons.map closes

# Use a marker that's unique: the reasons section uses dealSignal.reasons
# After it, there might be nothing or closing tags.
# Let me search for the pattern after reasons rendering

# Add the criteria table in a section that checks for criteriaMatches
criteria_section = '''
              {/* Criteria Match Breakdown */}
              {dealSignal.criteriaMatches && dealSignal.criteriaMatches.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {dealSignal.criteriaMatches.filter((m: any) => m.met).length}/{dealSignal.criteriaMatches.length} criteria met
                    </span>
                  </div>
                  <div className="grid gap-1.5">
                    {dealSignal.criteriaMatches.map((match: any, i: number) => (
                  ...
                  {!userCriteria && (
                    <p className="text-xs text-muted-foreground text-center pt-2">
                      Set up your <a href="/modeling/investment-criteria" className="text-primary underline">Investment Criteria</a> for personalized deal scoring
                    </p>
                  )}
                </div>
              )}
...
# Find the end of the signal card's CardContent and insert before it
# The signal card pattern: after the Key Factors div, there's </CardContent></Card>
# Let me find a unique closing pattern

# The signal card closing has:
#   </CardContent>
# </Card>
# within the IIFE

# Let me find the CardContent closing that's part of the signal card
# Unique marker: the signal card starts with dealSignal.borderColor 
# and ends with </CardContent>\n          </Card>

# Actually, I'll search for the end of reasons rendering
old_reasons_close = '''                    {dealSignal.reasons.map((reason, i) => (
                      <div key={i} className="flex items-start gap-2">'''

# Hmm, this is getting complicated without being able to see exact lines.
# Let me use a different approach: find the specific text after reasons
# and add the criteria section.

# The safest approach: find the signal card's closing CardContent tag
# There's a unique pattern: the score bar section followed by Key Factors followed by card close

# Let me look for the pattern right after "Key Factors" div closes
# The div has: cn("rounded-lg p-3 space-y-1.5", dealSignal.bgColor)
# and closes with </div>

# Use str_replace-like approach: find a unique anchor in the signal card
# The reasons section closes with ))} then </div> then </div>

# Actually, the CLEANEST way: insert the criteria section right before
# </CardContent> of the signal card. The signal card's CardContent 
# closing should be preceded by a specific pattern.

# Let me just find the score bar text and work from there
# I'll add the criteria grid between the score bar and key factors

old_score_labels = '''                    <span>Pass (0-49)</span>
...
new_score_labels = '''                    <span>Pass (0-49)</span>
                    <span>Conditional (50-69)</span>
                    <span>Buy (70-100)</span>
                  </div>
                </div>
...
if old_score_labels in dp and 'criteriaMatches' not in dp:
    dp = dp.replace(old_score_labels, new_score_labels, 1)
    changes += 1
    print("  OK Added criteria match grid in signal card")
elif 'criteriaMatches' in dp:
    print("  SKIP: Criteria matches already in signal card")
else:
...
''' + criteria_section + '''
                {dealSignal.reasons.length > 0'''
        dp = dp.replace(alt_old, alt_new, 1)
        changes += 1
        print("  OK Added criteria match grid (alt pattern)")
    else:
        print("  WARN: Could not find insertion point for criteria grid")
        print("  You may need to manually add the criteria grid to the signal card")

write(DP, dp)

# ================================================================
# 2. Create Investment Criteria Management Page
# ================================================================
print("\n=== 2. Create Investment Criteria management page ===")

criteria_page = '''import { useState, useEffect } from 'react';
...
page_path = "client/src/pages/modeling/investment-criteria.tsx"
if not os.path.exists(page_path):
    # Can't write directly, output to /mnt/user-data/outputs
    pass

write("/mnt/user-data/outputs/investment-criteria.tsx", criteria_page)
changes += 1
print("  OK Created investment-criteria.tsx page")
print("  → Copy to: client/src/pages/modeling/investment-criteria.tsx")
print("  → Add route in App.tsx: <Route path='/modeling/investment-criteria' component={InvestmentCriteria} />")

print(f"\n=== Criteria UI: {changes} patches ===")
