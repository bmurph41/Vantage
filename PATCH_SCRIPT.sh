#!/usr/bin/env bash
# ============================================================
# MarinaMatch AI Chatbot — Full Enhancement Patch
# Run from the Replit shell: bash /tmp/patch-ai-chatbot.sh
# ============================================================

set -e
echo "🤖 Patching MarinaMatch AI Chatbot..."

# ------------------------------------------------------------------
# 1. knowledge-base-service.ts  (NEW — was missing entirely)
# ------------------------------------------------------------------
cat > ~/workspace/server/services/knowledge-base-service.ts << 'KNOWLEDGE_EOF'
/**
...
echo "✅ knowledge-base-service.ts written"

# ------------------------------------------------------------------
# 2. Fix the replit_integrations routes — gpt-5.1 doesn't exist
# ------------------------------------------------------------------
sed -i 's/model: "gpt-5.1"/model: "gpt-4o"/g' ~/workspace/server/replit_integrations/chat/routes.ts 2>/dev/null || true
echo "✅ Fixed gpt-5.1 → gpt-4o in replit_integrations"

echo ""
echo "🎉 Core services patched. Next steps:"
echo "   1. Copy the enhanced ai-assistant-service.ts from the output files"
echo "   2. Copy the enhanced ai-assistant-routes.ts from the output files"
echo "   3. Copy the enhanced ai-assistant.tsx from the output files"
echo "   4. Restart the server"
echo ""
echo "   Schema tables will auto-create on first server start."
echo "   Global knowledge pool starts empty and grows from user feedback."
