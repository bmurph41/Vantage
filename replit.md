...
- **Modeling Projects**: Marina valuation, dynamic year selector, operations data sync, Exit Strategy Suite, AI-powered Document Intelligence (P&L/Rent Roll parsing, review workflow), Scenario Versioning, QuickBooks Integration, Capital Stack Builder, Pro Forma Engine, Portfolio Roll-ups. Enhanced with:
    - **P&L Pipeline (NEW)**: Hardened document extraction system with human-in-the-loop review, alias training bank, and full provenance tracking. Features: SHA256-based deduplication, job state machine (queued → processing → parsed → mapped → stored → completed/failed), canonical line item mapping with 0.75 confidence threshold for auto-mapping, review queue for low-confidence items, alias training that learns from human corrections, and fact storage with source traceability. Schema: 7 pnl_* tables (documents, jobs, parsed_statements, canonical_line_items, line_item_aliases, facts, review_items). Routes at `/api/pnl/*`. Frontend at `/modeling/pnl/upload` and `/modeling/pnl/review`.
