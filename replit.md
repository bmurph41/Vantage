...
- **Portfolio Dashboard** (Rebuilt): Institutional-grade 6-tab portfolio dashboard at `/modeling/portfolio`. Components: `client/src/pages/modeling/portfolio/index.tsx` (rebuilt, 6 tabs), `client/src/components/portfolio/PortfolioFilterBar.tsx` (multi-select filters: asset class, status, region, vintage year), `client/src/components/portfolio/AssetDetailDrawer.tsx` (right-side Sheet, 5 sub-tabs: Overview/Financials/Returns/Capital Stack/Documents). Tabs: (1) Overview — 6 hero KPIs (AUM, Equity, Debt, NOI, Cap Rate, IRR), asset class donut + T12 trend chart, deal status bars, NOI-by-class chart; (2) By Asset Class — collapsible Radix panels per class, asset table with click-to-drawer; (3) Returns — embeds existing `PortfolioReturns` component; (4) Individual Assets — sortable table with search, click-to-drawer; (5) Capital & Debt — maturity wall stacked bar + loan-level table; (6) Analytics — 5-factor scorecard gauges (RadialBarChart), 3 concentration risk donuts with HHI scores, benchmark comparison bar chart, HHI guide. New backend endpoints in `server/routes/portfolio-summary-routes.ts`: `GET /api/portfolio/debt-maturity-wall` (tranches grouped by maturity year, stacked by asset class), `GET /api/portfolio/concentration-risk` (HHI for asset class/geo/vintage), `GET /api/portfolio/scorecard` (5 scores 0–100), `GET /api/portfolio/assets/:projectId/detail` (full asset detail: project, T12 P&L, capital stack + tranches, returns/valuations, documents). DSCR derived from `capital_stacks.debt_yield`; orgId via `req.user?.orgId || req.tenantId`.
- **Pipeline Analytics Tab** (Task #20): Analytics view mode on the CRM pipeline board (`/pipeline/deal-board` → Analytics button). Single consolidated endpoint `GET /api/pipeline/analytics` (mounted at `/api/pipeline/analytics`) returns velocity, true stage-to-stage conversion rates (via `crm_deal_stage_history`), win/loss breakdown, avg deal size by stage, pipeline health score, and deal leaderboard. Frontend `PipelineAnalyticsView` consumes a single query to this endpoint. KEY: crm_deals uses `stage` (not `pipeline_stage`), `is_closed`/`lost_reason` (not `status`), `current_stage_entered_at` (not `stage_changed_at`), `title` (not `name`); crm_deal_stage_history has no `org_id` column.
- **Deal Comparison Workspace** (Task #15): Side-by-side deal comparison UI at `/pipeline/compare`. Consolidated API endpoint `GET /api/crm-v2/deals/compare-full?dealIds=id1,id2,...` returns financial model data, due diligence progress, CRM fields, activities, and computed IRR. Server: `server/routes/crm-enhancements-routes.ts`.
- **Workflow Email Send Integration** (Task #16): Email sending for CRM workflow sequences. DB tables: `workflow_email_templates`, `workflow_email_log` (created by migration 0012). Routes at `/api/workflow-email/`. Uses Resend API via `RESEND_API_KEY`.
- **AI Advisor Entity Injection** (Task #18): Context-aware AI assistant that receives live deal data (current deal, org analytics) when opened from a deal page. `DealContextResolver` service assembles and compresses entity context. Routes at `/api/ai-advisor/`.
- **Billing Engine** (Task #21): Subscription management with 4 tiers (starter/growth/institutional/enterprise). DB tables: `org_subscriptions`, `billing_events`, `billing_transactions`, `billing_seats`. `SUBSCRIPTION_TIERS` object exported from `server/services/billing-service.ts`. Routes at `/api/billing/`. Requires `STRIPE_SECRET_KEY` (warning displayed if missing but app continues).
- **Dynamic Asset Classes** (Task #22): DB-driven asset class registry. Table: `platform_asset_classes` (36 canonical types seeded at startup). Migration `migrations/0005_asset_class_dynamic_fields.sql` converts key/category from enum to text, adds metadata columns. `AssetClassService` with startup seeder. Routes at `/api/asset-classes`. Category groups: marina, commercial, residential, hospitality, specialty.
