...
- **Document Processing**: P&L Pipeline V2 for document extraction with human-in-the-loop review, OCR and LLM classification layers, and a marina-specific Chart of Accounts. LLM classifiers (OpenAI/Anthropic) use the platform's 9-department taxonomy (Storage, Fuel, Ship's Store, Service, Boat Sales, Boat Brokerage, Payroll, Marina & Amenities, General) with section-context hints for disambiguation. Department and bucket normalization ensures consistent naming across all pipeline stages. Global keyword learning: user-approved classifications create global rules (orgId=null) that improve accuracy for all users over time. Rent Roll Document Parser unifies CSV/Excel/PDF parsing with AI and heuristic fallbacks.
- **COA-Based Financial Normalization Engine**: Full Chart of Accounts management with CSV import, manual entry, and tree view. Category Mapping maps COA accounts to 69 seeded mm_standard_accounts via: locked COA mappings → AI suggestions (above confidence threshold) → rule-based keyword fallback (30+ marina-specific keywords) → unmapped. Non-blocking post-parse normalization trigger fires after P&L document extraction. Feature-gated by FEATURE_COA_MAPPING env var. Tables: chart_of_accounts, coa_mapping, mm_standard_accounts, coa_mapping_suggestions, financial_line_items_normalized, coa_audit_log. Routes: /api/coa/*, /api/financial-normalization/*. Frontend: Chart of Accounts, Category Mapping, Normalization Status pages under /modeling/settings/.
- **Pro Forma Engine**: Institutional-grade P&L waterfall (Revenue → COGS → Gross Profit → OpEx → NOI) with year-specific granular growth rates at department and line-item level. Inline Assumptions Editor allows adjusting growth rates directly on the Pro Forma page via popover-based editor on department rows (annual view), with bidirectional sync to Assumptions tab through shared scenario JSON and query key invalidation. Below-the-line cash flow calculations (management fee, CapEx, reserves, debt service → levered cash flow). Exit year waterfall (selling fees, loan payoff, exit fees, working capital recovery → net exit proceeds). IRR calculated on true levered cash flows with total equity invested. Historical P&L separates COGS from operating expenses. Assumptions stored in scenario JSON: `yearlyGrowthRates`, `lineItemOverrides`, `belowTheLine`, `exitAssumptions`. T12-Aware Year 1 Logic: When a T12 is the most recent data, the engine supports two modes (`year1Mode` in display preferences): `calendar_year_end` (Year 1 = calendar year, T12-covered months are Actual, remaining are Forecast) and `next_12_months` (Year 1 = rolling 12 months from T12 end, all years roll). Monthly projections include `isActual`/`isForecast` flags for visual distinction. Period building via `buildT12AwarePeriods()` in `server/utils/modeling-periods.ts`.
...
- **Returns Module**: Institutional-grade return tracking at model and portfolio levels. Canonical ledger (`returns_ledger`) with 12 bucket types (ACQUISITION, EQUITY_CONTRIBUTION, LOAN_PROCEEDS, OPERATING_CASHFLOW, CAPEX, DEBT_SERVICE_*, REFI_PROCEEDS, SALE_*, LOAN_PAYOFF, FEES_OTHER). Sign convention: outflows negative, inflows positive. Valuation timeline (`returns_valuation`) and loan balance timeline (`loan_balance_timeline`). Metrics: MOIC, ROI, XIRR (Newton-Raphson + bisection), Gross Gain, attribution waterfall. Levered/Unlevered views with equity vs all-in spend toggles. Service: `server/services/returns-service.ts`. Routes: `/api/returns/model/:modelId`, `/api/returns/property/:propertyId`, `/api/returns/portfolio`. Seed endpoint: `POST /api/returns/seed/:modelId`. Frontend: "Returns" tab in Financial Model workspace, Portfolio Returns page at `/modeling/portfolio/returns`.
