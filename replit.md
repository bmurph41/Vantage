...
- **Pro Forma Engine**: Institutional-grade P&L waterfall (Revenue → COGS → Gross Profit → OpEx → NOI) with year-specific granular growth rates at department and line-item level. Below-the-line cash flow calculations (management fee, CapEx, reserves, debt service → levered cash flow). Exit year waterfall (selling fees, loan payoff, exit fees, working capital recovery → net exit proceeds). IRR calculated on true levered cash flows with total equity invested. Historical P&L separates COGS from operating expenses. Assumptions stored in scenario JSON: `yearlyGrowthRates`, `lineItemOverrides`, `belowTheLine`, `exitAssumptions`.
