...
- [spec] [complete] Write detailed build spec for Deal Comparison workspace — see agents/specs/deal-comparison-workspace-spec.md
- [feature] [todo] Build Deal Comparison in Workspace — see agents/specs/deal-comparison-workspace-spec.md (CRM priority #3)
...
- [feature] [done] Fix New Deal button + build deal-creation wizard — New Deal button on Deal Workspace is fully non-functional. Build a multi-step wizard for deal creation covering all basics (likely: deal name, asset class, property linkage, key dates, team), then save and route to the new deal's workspace. Sub-tasks: diagnose root cause of broken button, design wizard steps, build wizard, integrate with existing deal CRUD endpoints, verify workspace auto-initialization on save. BETA-BLOCKER. Discovered during user beta self-testing 2026-04-27. — *Verified 2026-04-27: Replit Agent commit 5a8cdc58 fixed root cause — DealFormModal in deal-workspace.tsx was being passed prop names `open`/`onOpenChange` but component expected `isOpen`/`onClose`. Modal never opened on button click. 2-line fix; modal now opens correctly. Wizard flow itself was already built and working — the button just couldn't trigger the modal due to prop mismatch.*

- [feature] [in-progress] Make Financial Model New Project wizard asset-class-aware — wizard is currently hardcoded marina-specific. Drive wizard fields and behavior from ASSET_REGISTRY (extends Phase 3A foundation). Remove redundant 'Ownership' section (verify what's redundant before deleting). Verify wizard correctly populates Modeling Project + linked CRM Deal + CRM Property records (name, address, asset class, key dates flow to all three). Should integrate with shared/asset-class-model-config.ts (currently part of 824 typecheck baseline errors). Discovered during user beta self-testing 2026-04-27. — *Partial progress 2026-04-27: Replit Agent commit 035164b9 did surface-level UX work — de-marina'd labels (Marina Name → Property/Location Name, Slip/Berth → Unit/Suite/Berth, etc.), Anchor icon → Briefcase/Building2, generic Quick Start templates (Acquisition, Lease, New Listing, Business Deal), conditional rendering of Physical Capacity / Facilities sections only for marina-related asset types, dialog visual de-colorization. STILL PENDING: ASSET_REGISTRY-driven field configuration; integration with shared/asset-class-model-config.ts (currently part of 824 typecheck baseline errors); removal of redundant Ownership section; verification that wizard correctly populates Modeling Project + linked CRM Deal + CRM Property records (name, address, asset class, key dates flow to all three). Approximately 80% of visible UX done; underlying model-driven behavior still needed.*
...
- [security] [in-progress] Audit all route files for missing orgId filters — Grep all `server/routes/` for `.where(eq(...id, req.params...))` patterns that lack an accompanying orgId condition on org-scoped tables; fix each occurrence — *A9-1 audit complete 2026-04-27, see ROUTE_TENANT_AUDIT.md (290 lines). 519 route candidates + 412 service candidates flagged by pattern scan; SEV-1 SQL injection found in distribution-approval-service.ts (3 sites, lines 338/371/398); SEV-2 post-fetch orgId checks in fund-management-routes.ts capital-account routes. A9-2 fix work pending.*

- [security] [todo] A9-2: Fix routes and services flagged as vulnerable in ROUTE_TENANT_AUDIT.md — see audit document for prioritized list. Phase 1 (SEV-1, ~2h): fix 3 SQL injection sites in distribution-approval-service.ts (raw template-literal SQL with user-controlled inputs cast as `any`). Phase 2 (SEV-2, ~1h): refactor 5 capital-account routes in fund-management-routes.ts to hoist atomic fund-org verification above account fetch. Phase 3 (SEV-3 triage, L-XL): manual line-by-line review of high-flag-count files (crm-routes 51, budget-routes 25, broker-dashboard 24, infrastructure 22, crm-summary 17, operations-context 16, dd-routes 14, modeling-routes 14) to separate true vulnerabilities from parent-validation false-positives.

### 0C — Transaction Safety for Financial Operations

- [security] [done] Wrap distribution execution in db.transaction() — `server/services/distribution-approval-service.ts` `execute()` method performs 4+ sequential DB operations (draft lookup, compliance checks, fund distribution processing, draft update, audit log) without transaction; wrap in `await db.transaction(async (tx) => { ... })` passing tx to each step — *Verified 2026-04-27 during A9-1 audit: execute() at line 233 already wraps the full sequence in `await db.transaction(async (tx) => { ... })`. Audit stale, item already fixed. (Note: SEV-1 SQL injection in adjacent methods still pending — see ROUTE_TENANT_AUDIT.md.)*
